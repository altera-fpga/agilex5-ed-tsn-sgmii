...
// register offset : 0x40, field offset : 0, access : RO, SFP_STATUS.sfp28_int_i
input   SFP_STATUS_sfp28_int_i,
// register offset : 0x40, field offset : 1, access : RO, SFP_STATUS.sfp28_tx_fault_i
input   SFP_STATUS_sfp28_tx_fault_i,
// register offset : 0x40, field offset : 2, access : RO, SFP_STATUS.sfp28_los_i
input   SFP_STATUS_sfp28_los_i,
// register offset : 0x40, field offset : 3, access : RO, SFP_STATUS.sfp28_mod_det_i
input   SFP_STATUS_sfp28_mod_det_i,
// register offset : 0x40, field offset : 0, access : RW, SFP_CTRL.sfp28_tx_disable_o
output reg SFP_CTRL_sfp28_tx_disable_o,
...
// register SFP with  writeType: write
wire	  we_SFP		=	we  & (addr[6:0]  == 7'h44)	?	byteenable[0]	:	1'b0;
...
always @( posedge clk)
   if (!reset_n)  begin
      SFP_CTRL_sfp28_tx_disable_o <= 1'h0;
   end
   else begin
   if (we_SFP) begin 
      SFP_CTRL_sfp28_tx_disable_o   <=  din[0];  //
   end
end
...
rdata_comb = 32'h00000000;
   if(re) begin
	...
	7'h40 : begin
		rdata_comb [0]	= SFP_STATUS_sfp28_int_i ;  // readType = read   writeType =illegal
		rdata_comb [1]	= SFP_STATUS_sfp28_tx_fault_i ;  // readType = read   writeType =illegal
		rdata_comb [2]	= SFP_STATUS_sfp28_los_i ;  // readType = read   writeType =illegal
		rdata_comb [3]	= SFP_STATUS_sfp28_mod_det_i ;  // readType = read   writeType =illegal
		rdata_comb [31:4]	= 32'h00000000 ;  // SFP_STATUS_Reserved 	is reserved or a constant value, a read access gives the reset value
	end
	7'h44 : begin
		rdata_comb [0]	=  SFP_CTRL_sfp28_tx_disable_o ;		// readType = read   writeType =write
		rdata_comb [31:1]	= 31'h0000000 ;  // SFP_CTRL_Reserved 	is reserved or a constant value, a read access gives the reset value
	end
	default : begin
		rdata_comb = 32'h00000000;
	end
