...
module alt_mge_phy_xgmii_pcs #(
  ...
  parameter ENABLE_IEEE1588  = 0
	...
	// + 1 fast clock cycle for case:433815              = (xgmii_clk_cycle: 0.30303) 
    //------------------------------------------------------------------
    // PCS latency in cycles (exc. soft-FIFO)           = 10.9387013
    //------------------------------------------------------------------
	...
	// - soft-FIFO's latency in cycles (latency_quo)    = 12'h136   (xgmii_clk_cycle: 4.84375)
	// - AIB FIFO latency (mid-point)                   = 16'h1a36  (xgmii_clk_cycle: 2.9734847)
	// + 1 fast clock cycle for case:433815              = (xgmii_clk_cycle: 0.30303) 
	 //------------------------------------------------------------------
    // PCS latency in cycles (exc. soft-FIFO & aib fifo)  = 12.0406391 
    //------------------------------------------------------------------
	
    localparam TX_OFFSET = (DEVICE_FAMILY == "Arria 10")? ((PMA_MODE == 32) ? {6'd10,10'd848}    // 10.82859375 cycles
                                                           : {6'd12,10'd289})   // 12.28199219 cycles
                                                           : {6'd12,10'd41}; 	  // 12.018123  cycles												   
