...
// --------------------------------------------------------------------
`timescale 1ns/1ns
module alt_mge16_pcs_1588_ppm_counter
 #(
  ...
  (
    ...
    input  wire                     clk_a_sync_status, // word-aligner lock signal (sync-ed to clock a)
...
// run cntr_a (initiator) only if both rst_a_n and rst_b_n are releaased, and word-aligner lock is up.
// before word-aligner lock is up, clk_b is unstable and result in inaccurate ppm value.
assign rst_a_n_final_flop = rst_a_n & ~sync_rst_b & clk_a_sync_status;
...
alt_mge16_pcs_reset_synchronizer reset_sync_3 (
    .clk(clk_a),
    .reset_in(~rst_b_n),
...
// clk_a counter
// run cntr_a (initiator) only if both rst_a_n and rst_b_n are releaased
...
alt_mge_phy_std_synchronizer_bundle #(2, sync_stages) counter_run_transfer(
    .clk(clk_b),
    .reset_n(rst_b_n),
...
alt_mge16_pcs_clock_crosser cntr_b_clock_crosser(
                                .in_clk(clk_b),
                                .in_reset(~rst_b_n),
                                .in_ready(),
...
defparam
cntr_b_clock_crosser.SYMBOLS_PER_BEAT = 1,
cntr_b_clock_crosser.BITS_PER_SYMBOL = PPM_CNTR_WIDTH;
