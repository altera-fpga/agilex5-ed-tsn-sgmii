...
/***********************************************************************************
module adder_36 is the 36 bit binary counter.
start: enbale in
d_out : data output
...
module alt_mge_phy_f_ptp_latency_count_async #(
...
localparam  START_CNT        = 2'b0;
localparam  STOP_CNT         = 2'b1;

localparam ASYNC_CNT_LOCAL = DL_TOT_MEASURE_CNT << 1;
...
assign stop_pulse = stop_d3 & (!stop_d2); // detect falling edge for stop signal
...
always @(posedge clk) begin
    if (reset == 1'b1) begin
        ...
        cnt_state <= START_CNT;
    end else if (stop_pulse_cnt <=  (DL_TOT_MEASURE_CNT << 1)) begin
        case (cnt_state)
            START_CNT: begin
