...
// Custom Cadence Controller

module ftile_efifo_ccc #(
    parameter SIM_EMULATE           = 1'b0,                             // 0
    parameter WIDTH                 = 80,                               // typical 20,40,60,80
    parameter ADDR_WIDTH            = 3,                                // support 3|5 only. Use 5 when SYNC_STAGES > 2
...
wire waclr, raclr, waclr_n, raclr_n;
    alt_xcvr_resync_std #(
        ...
        ) __syncreset__rst_sync_tx_rd_clk (
            .clk    (rclk),
            .reset  (aclr),
            .d  (1'b1),
    ...
    alt_xcvr_resync_std #(
            .SYNC_CHAIN_LENGTH(3),    .WIDTH(1),  .INIT_VALUE(0)    
        ) __syncreset__rst_sync_tx_wr_clk (
            .clk    (wclk),
            .reset  (aclr),
            .d  (1'b1),
...
generate
    if (ADDR_WIDTH == 3) begin : a3gc
        // gray write pointer
        tse_ftile_gray_cntr_3 wcntr (
            .clk(wclk),
            .ena(winc),
            .aclr(waclr),
            .cntr(waddr_g)
		...
		// gray write pointer
		tse_ftile_gray_cntr_5_sl wcntr (
			.clk(wclk),
			.ena(winc),
			.sld(waclr),
			.cntr(waddr_g)
        ...
        defparam wcntr .SIM_EMULATE = SIM_EMULATE;
        
		// gray read pointer
		tse_ftile_gray_cntr_5_sl rcntr (
			.clk(rclk),
			.ena(rinc),
			.sld(raclr),
			.cntr(raddr_g)
        ...
        defparam rcntr .SIM_EMULATE = SIM_EMULATE;
    end
endgenerate
...
tse_ftile_sync_regs_aclr_m2 sr0 (
    .clk(rclk),
    .aclr(raclr),
...
tse_ftile_sync_regs_aclr_m2 sr1 (
    .clk(wclk),
    .aclr(waclr),
...
generate
    if (PREVENT_OVERFLOW) begin : po
        tse_ftile_neq_5_ena eq2 (
            .da(5'h0 | wside_raddr_g_completed),
            .db(5'h0 | waddr_g),
            .ena(wreq),
            .eq(winc)
...
generate 
    if (PREVENT_UNDERFLOW) begin : pu
        tse_ftile_neq_5_ena eq3 (
            .da(5'h0 | raddr_g_completed),
            .db(5'h0 | rside_waddr_g_completed),
            .ena(rreq),
            .eq(rinc)
