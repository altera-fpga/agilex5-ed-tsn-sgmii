...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [alt_em10g32_0::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {alt_em10g32_0::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [alt_em10g32_0::get_design_files "$QSYS_SIMDIR"]]
...
run_vcs="vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  ...
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_hvio.sv \
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_hvio_ncrypt.sv \
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_agilex5_io96.sv \
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_agilex5_io96_ncrypt.sv \
  $QUARTUS_INSTALL_DIR/eda/sim_lib/tennm_atoms.sv \
  $QUARTUS_INSTALL_DIR/eda/sim_lib/synopsys/tennm_atoms_ncrypt.sv \
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_agilex5_hssi_a.sv \
  $QUARTUS_INSTALL_DIR/../devices/sim_lib/tennm_agilex5_hssi_a_ncrypt.sv \
  $common_design_files \
  $design_files \
  $USER_DEFINED_ELAB_OPTIONS_APPEND \
  -top $TOP_LEVEL_NAME
