...
`timescale 1ns / 1ns
module alt_em10g32_rx_gmii_mii_decoder_if (

    reset,                  //INPUT :  Reset
    rx_clk,                 //INPUT :  MII Clock
    clk_ena,                //INPUT :  MII Clock Enable
    clk_ena_half_rate,      //INPUT :  MII Clock Enable (Half rate)
    mii_rxd,                //INPUT :  MII receive data
    mii_rxdv,               //INPUT :  MII receive frame enable  
    mii_rxerr,              //INPUT :  MII receive frame error
    mii_rxd_o,              //OUTPUT:  MII receive data
    mii_rxdv_o,             //OUTPUT:  MII receive frame enable  
    mii_rxerr_o,            //OUTPUT:  MII receive frame error
    mii_align_err,          //OUTPUT:  MII receive alignment error
    mii_alignment_status    //OUTPUT:  Status of the alignment for latency calculation
);
   
input   reset;                  //  Reset
input   rx_clk;                 //  MII Clock
input   clk_ena;                //  MII Clock Enable
input   clk_ena_half_rate;      //  MII Clock Enable (Half rate)
...
output  mii_align_err;          //  MII receive alignment error
...
assign mii_align_err = alignment_error;
...
always @(posedge rx_clk) begin
    if(reset) begin
        ...
        if(clk_ena) begin
            mii_clk_ena <= clk_ena & clk_ena_half_rate;
        end
    end
end
    ...
    if(reset) begin
        ...
        if(clk_ena & mii_clk_ena) begin
            ...
            if(!packet_in_progress) begin
                ...
                if(mii_rxdv_reg_2 && (mii_rxd_reg_1 == 4'hD)) begin
                ...
                else if(mii_rxdv_reg_1 && (mii_rxd_reg_0 == 4'hD)) begin
                    packet_in_progress <= 1'b1;
                    align_pipeline_1_2 <= 1'b0;
            ...
            // After alignment is done, continuously output data
                ...
                if(align_pipeline_1_2 == 1'b1) begin
                    if(mii_rxdv_reg_1 & mii_rxdv_reg_2) begin
                        ...
                        mii_rxd_o <= {mii_rxd_reg_1, mii_rxd_reg_2};
                        mii_rxdv_o <= 1'b0;     // Truncate to nearest octet
                        mii_rxerr_o <= mii_rxerr_reg_1 | mii_rxerr_reg_2;
                    end
                    else begin
                        packet_in_progress <= 1'b0;
