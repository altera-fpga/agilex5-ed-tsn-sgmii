...
module alt_em10g32_tx_rs_xgmii_layer #(
    ...
    output reg tx_packet_in_progress_xgmii,
...
(* altera_attribute = {"-name SYNCHRONIZER_IDENTIFICATION OFF"} *) reg     [31:0]eth_pkt_data_10g;
reg     [31:0]eth_pkt_data_10g_dly1;
reg     [31:0]eth_pkt_data_10g_dly2;
(* altera_attribute = {"-name SYNCHRONIZER_IDENTIFICATION OFF"} *) reg     [3:0]eth_pkt_ctrl_10g;
...
assign state_error = mx2rs_ethfrm_error;
assign state_data = mx2rs_ethfrm_revdata_dly1;

assign state_arc_data_crc_dly1 = PREAMBLE_PASSTHROUGH?arc_DATA_CRC_dly4:arc_DATA_CRC_dly2;
assign state_arc_data_crc = PREAMBLE_PASSTHROUGH?arc_DATA_CRC_dly3:arc_DATA_CRC_dly1;
assign state_eth_pkt_data_10g = PREAMBLE_PASSTHROUGH?eth_pkt_data_10g_dly2:eth_pkt_data_10g;
assign state_eth_pkt_ctrl_10g = PREAMBLE_PASSTHROUGH?eth_pkt_ctrl_10g_dly2:eth_pkt_ctrl_10g;
...
generate if(ENABLE_10GBASER_REG_MODE == 1)
begin
reg [4:0]counter;
reg counter2;
...
else if(ENABLE_1G10G_MAC == 5)
begin
// rs2top_eth_xgmii_valid counter. this register will control valid assertion based on the selected speed.
// for 10G, valid will always high
// for 5G, valid will toggle
// for 2.5G, valid will asserted high 1 clock cycle for every 4 clock cycle
// for 1G, valid will asserted high 1 clock cycle for every 10 clock cycle
// for 100M, valid will asserted high 1 clock cycle for every 100 clock cycle
// for 10M, valid will asserted high 1 clock cycle for every 1000 clock cycle

reg [1:0]counter_2_5g;
reg [3:0]counter_1g;
reg [6:0]counter_100m;
reg [9:0]counter_10m;
...
always @ (posedge clk)
    ...
    if(!rst_n)
        ...
        3'b100: begin
                // assertion counter_1g only between 0 until 9
                //xgmii_tx_valid_cycles: assert property ( @(posedge `ETH_TB_TOP.U_TOP_DUT_WRAPPER.U_DUT_WRAPPER.U_DUT.tx_xcvr_clk) )
                //: assert property ( @(posedge clk) disable iff (!rst_n) `rs2top_eth_xgmii_valid##1`!rs2top_eth_xgmii_valid |=> (`!rs2top_eth_xgmii_valid)[*3]##1`rs2top_eth_xgmii_valid) 
                //else $error("rs2top_eth_xgmii_valid out of range");
                if(counter_2_5g == 2'b11)
                    begin
                    counter_2_5g <= 2'b00;
                    rs2top_eth_xgmii_valid <= 1'b1;
                    end
                    ...
                    begin
                    counter_2_5g <= counter_2_5g + 1'b1;
                    rs2top_eth_xgmii_valid <= 1'b0;
                    end
                end
        3'b001: begin
                // assertion counter_1g only between 0 until 9
                //xgmii_tx_valid_cycles: assert property ( @(posedge `ETH_TB_TOP.U_TOP_DUT_WRAPPER.U_DUT_WRAPPER.U_DUT.tx_xcvr_clk) )
                //: assert property ( @(posedge clk) disable iff (!rst_n) counter_1g inside {[0:9]}) 
                //else $error("counter_1g out of range");
                if(counter_1g == 4'b1001)
                    begin
                    counter_1g <= 4'b0;
                    rs2top_eth_xgmii_valid <= 1'b1;
                    end
                    ...
                    begin
                    counter_1g <= counter_1g + 1'b1;
                    rs2top_eth_xgmii_valid <= 1'b0;
                    end
                end
        3'b010: begin
                if(counter_100m == 7'b1100011)
                    begin
                    counter_100m <= 7'b0;
                    rs2top_eth_xgmii_valid <= 1'b1;
                    end
...
// hold the empty. We do not need to precisely calculate when to hold the ready, as long as it is update during EOP then is enough
// When to use this empty value will be take care by state machine
// SYNC_RESET FLOPS
always @ (posedge clk)
	begin
	if(!rst_n)
...
always @ (posedge clk)
	...
	if(!rst_n)
        ...
        if(mx2rs_sop && mx2rs_valid && PKT_10G_SM_ns == PREAM1 && rs2top_eth_xgmii_valid)
            ...
            begin
            buff_frm_type_hold <= buff_frm_type_hold;
            end
		end
	end    
    

// need to make sure after EOP, need to back pressure buffer
// SYNC_RESET FLOPS
always @ (posedge clk)
	begin
	if(!rst_n)
        ...
        else if (PKT_10G_SM_ps == GAP && rs2top_eth_xgmii_valid)
            begin
            after_eop_hold <= 1'b0;    
            end
        else
            begin
            after_eop_hold <= after_eop_hold;
            end
		end	
	end    
...
always @ (posedge clk)
	...
	// else
        ...
        if(rs2top_eth_xgmii_valid)
            ...
            begin
            mx2rs_ethfrm_revdata_dly1 <= mx2rs_ethfrm_revdata_dly1;
            end
		end
	end    
...
always @ (posedge clk)
    ...
    if(!rst_n)
        ...
        if(rs2top_eth_xgmii_valid)
            ...
            if(rs2mx_ethfrm_ready)	
                ...
                begin
                mx2rs_ethfrm_sop_dly2 <= mx2rs_ethfrm_sop_dly2;
                end
    ...
    generate 
    if ((DEVICE_FAMILY == "Stratix 10") || (DEVICE_FAMILY == "Agilex 5"))
        begin
        
        wire [31:0]	        buff_data_pipe1;
        wire				buff_valid_pipe1;
        wire				buff_startofpacket_pipe1;
        wire				buff_endofpacket_pipe1;
        wire	[1:0]		buff_empty_pipe1;
        wire	[1:0]		buff_frm_type_pipe1;
        wire				in_ready_p2;
        ...
        alt_em10g32_rr_buffer #(
           .SYNC_RESET_N(SYNC_RESET_N)
            ...
            // source
            .out_data   (buff_data_pipe1),
            .out_valid  (buff_valid_pipe1),
            .out_ready  (in_ready_p2),
            .out_startofpacket  (buff_startofpacket_pipe1),
            .out_endofpacket    (buff_endofpacket_pipe1),
            .out_empty  (buff_empty_pipe1),
            .out_error  (),
            .out_channel (buff_frm_type_pipe1)
        ); 
        
        alt_em10g32_pipeline_base #(
            .BITS_PER_SYMBOL    (32 + 1 + 1 +2 + 2),
            .SYMBOLS_PER_BEAT   (1),
            .PIPELINE_READY     (0)
            ) buffer_1 (
            .clk        (clk),
            .reset_n    (rst_n),
            .in_ready   (in_ready_p2),
            .in_valid   (buff_valid_pipe1),
            .in_data    ({buff_data_pipe1,buff_startofpacket_pipe1,buff_endofpacket_pipe1,buff_empty_pipe1,buff_frm_type_pipe1}),
            .out_ready  (buff_ready & rs2top_eth_xgmii_valid),
            .out_valid  (buff_valid),
            .out_data   ({buff_data, buff_startofpacket, buff_endofpacket, buff_empty, buff_frm_type})
        );
    ...
    //insert buffer. Can consider this buffer as swallow SC FIFO
    alt_em10g32_rr_buffer buffer_inst(
    ...
    assign buff_ready = (  (PKT_10G_SM_ps == IDLE & !(mx2rs_sop & mx2rs_valid)) 
                        | (PKT_10G_SM_ps == PREAM1)
                        | ((PKT_10G_SM_ps == DATA | PKT_10G_SM_ps == DATAWCRC) & !after_eop_hold) 
                        | ((cr_preamble_passthru & mx2rs_frm_type[1] != 1'b1) & (PKT_10G_SM_ps == IDLE | PKT_10G_SM_ps == PREAM0 | (PKT_10G_SM_ps == GAP & ipgphcnt_eq0 & mx2rs_sop & mx2rs_valid))))?1'b1:1'b0;
    ...
    always @ (posedge clk)
        ...
        if(!rst_n)
            ...
            case({(mx2rs_valid && mx2rs_eop && rs2top_eth_xgmii_valid),ipg_insrt_done})
            ...
            case({(mx2rs_valid && rs2top_eth_xgmii_valid && mx2rs_eop),(PKT_10G_SM_ps == GAP | PKT_10G_SM_ps == EFD)})
            ...
            if(rs2top_eth_xgmii_valid)
                ...
                begin
                int_eop_holdreg_dly <= int_eop_holdreg_dly;
                end
            end             
        end
    ...
    always @ (posedge clk)
     ...
     always @ (*)
        ...
        DATA:       if((int_eop && rs2top_eth_xgmii_valid))      // if valid deasserted during data
                        begin
                        PKT_10G_SM_ns = CRC;
                        end
                    else
    ...
    assign ipg_insrt_done = PKT_10G_SM_ps == GAP && ipgphcnt_eq0;
        ...
        if(!rst_n)
            ...
            if(rs2top_eth_xgmii_valid)
                ...
                DATA    :   begin  
                            ...
                            if(mx2rs_ethfrm_pre_error != 0 || state_error != 0)
                                begin
    ...
    always @ (posedge clk)
        ...
        if(!rst_n)
            begin
            tx_packet_in_progress_xgmii <= 1'b0;
            end
        else    
            begin
            if(PKT_10G_SM_ns == PREAM0)
                begin
                tx_packet_in_progress_xgmii <= 1'b1;
                end
            else if (detect_EFD)  
                begin
                tx_packet_in_progress_xgmii <= 1'b0;
