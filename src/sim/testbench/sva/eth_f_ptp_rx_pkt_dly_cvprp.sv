...
`altuvm_sva_cover(SPTP_SOP_DURING_RX_VALID0, (!i_rx_valid && (|inc_rx_sop)), (posedge i_clk), (!i_rst_n) )
`altuvm_sva_cover(SPTP_EOP_DURING_RX_VALID0, (!i_rx_valid && (|inc_rx_eop)), (posedge i_clk), (!i_rst_n) )

generate
for (i=0; i<WORDS; i++) begin:cvp_word
`altuvm_sva_cover(SPTP_RX_EOP_AT_WORDSn, (i_rx_valid && inc_rx_eop[i]), (posedge i_clk), (!i_rst_n) )
end

if (WORDS==16) begin: cvp_e400g
`altuvm_sva_cover(SPTP_TWO_RX_PKT_IN_ONE_CYCLE_400G, (loword_sop && hiword_sop && loword_eop && hiword_eop), (posedge i_clk), (!i_rst_n) )
end
endgenerate
