...
class an_reconfig_reset_during_an_sequence extends an_base_sequence;
   `uvm_object_utils(an_reconfig_reset_during_an_sequence)
     int num_reset;
     int num_pages_sent;
     bit vip_np=0;
     bit reset_in_middle_of_an;

      
   function new(string name = "an_reconfig_reset_during_an_sequence");
      super.new(name);
...
`endif
  ...
  `endif   
   ...
   task automatic an_reconfig_reset(speed_e speed, int node, int inst);
      string func_name = "an_reconfig_reset_task";
      ...
      //Revisit: vinoth2x - make the num_reset to 1 and reset_in_middle_of_an to 1 until RTL is stable 
      //num_reset = $urandom_range(1,5);
      //reset_in_middle_of_an=$urandom_range(0,1);
      num_reset = 1;
      reset_in_middle_of_an = 1;
      if (vip_np) begin
       p_sequencer.top_env.env_ip[inst].ts_tasks_if.do_drv_cfg(`ETH_MODE_AN_NEXT_PAGE_BIT,vip_np);
      end
      `uvm_info(get_type_name(), $sformatf("%s: Perform reset %0d times",func_name,num_reset), UVM_NONE);
      // Wait AN in progress
      for (int i=0;i<num_reset;i++) begin
	 fork : wait_an
	    begin
	       if(reset_in_middle_of_an) begin
		  `uvm_info(get_type_name(), $sformatf("%s: Reset in the middle of AN",func_name), UVM_NONE); 
		  if (!vip_np)
		    num_pages_sent = $urandom_range(1,10);
		  else
		    num_pages_sent = $urandom_range(8,16);
		  repeat(2*num_pages_sent)
		    @ (p_sequencer.top_env.env_ip[inst].m_snps_eth_pcs66_agent.vif.debug_bus.debug_bus_an.debug_bus_an73_bfm.arbiter_dme_page_received==1);
	       end else begin
		  `uvm_info(get_type_name(), $sformatf("%s: Reset after AN complete",func_name), UVM_NONE); 
	    ...
	    begin
	       #2ms;
	       `uvm_fatal(get_type_name(), $sformatf("%s: Timeout waiting for AN",func_name));
	       disable wait_an;	        
	    end
	 join
	 // csr reset
         p_sequencer.top_env.env_ip[inst].apply_reconfig_reset();
         `uvm_info(get_type_name(), $sformatf("vnot reset applied"), UVM_NONE);
	 p_sequencer.top_env.disable_an_snps_errors(speed,inst);
	 check_reset_values(speed,node,inst);
         `uvm_info(get_type_name(), $sformatf("vnot1 reconfig AN"), UVM_NONE);
	 reprogram_an_after_reset(speed,node,inst);
         ...
         `uvm_info(get_type_name(), $sformatf("vnot1 CPU moved to RC state"), UVM_NONE);
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_reconfig_reset_during_an_sequence_body";
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 10G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_10g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 25G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_25g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 40G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_40g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 50G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_50g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 100G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_100g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 200G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_200g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 400G - an_reconfig_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_400g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]), UVM_NONE);
                     an_reconfig_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
...
endclass:an_reconfig_reset_during_an_sequence
