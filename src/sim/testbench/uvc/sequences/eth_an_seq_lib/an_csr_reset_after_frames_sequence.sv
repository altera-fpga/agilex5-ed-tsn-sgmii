...
class an_csr_reset_after_frames_sequence extends an_base_sequence;
   `uvm_object_utils(an_csr_reset_after_frames_sequence)
     int num_reset;
   int 	 num_pages_sent;

      
   function new(string name = "an_csr_reset_after_frames_sequence");
      super.new(name);
...
`endif
  ...
  `endif   
   ...
   task automatic an_csr_reset(speed_e speed, int node, int inst);
	 ...
	 // csr reset
         ...
         wait(p_sequencer.top_env.env_ip[inst].m_snps_eth_pcs66_agent.vif.reset == 0);
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_csr_reset_after_frames_sequence_body";
...
endclass:an_csr_reset_after_frames_sequence
