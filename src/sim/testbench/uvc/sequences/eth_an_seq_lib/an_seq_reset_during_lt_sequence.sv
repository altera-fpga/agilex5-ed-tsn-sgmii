...
class an_seq_reset_during_lt_sequence extends an_base_sequence;
   `uvm_object_utils(an_seq_reset_during_lt_sequence)
     int rand_delay;

   
   function new(string name = "an_seq_reset_during_lt_sequence");
      super.new(name);
...
`endif
  ...
  `endif
   ...
   task automatic an_seq_reset(speed_e speed, int node, int inst);
      ...
      `uvm_info(get_type_name(), $sformatf("400us Delay between AN and LT"), UVM_NONE);
	 ...
	 begin
      ...
      join
      // Reset SEQ
      `uvm_info(get_type_name(), $sformatf("%s: Resetting Sequencer",func_name), UVM_LOW)
      read_data=32'b0;
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_seq_reset_during_lt_sequence_body";
...
endclass:an_seq_reset_during_lt_sequence
