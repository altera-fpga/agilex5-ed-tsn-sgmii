...
class an_restart_during_an_sequence extends an_base_sequence;
   `uvm_object_utils(an_restart_during_an_sequence)
     int num_reset;
     int num_pages_sent;
     bit vip_np=0;

      
   function new(string name = "an_restart_during_an_sequence");
      super.new(name);
...
`endif
  ...
  `endif   
   ...
   task automatic restart_an(speed_e speed, int node, int inst);
	 ...
	 p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_cfg2"),read_data,speed,.disable_check(1'b1));
	  // Revisit: vinoth2x - check this reg field in GDR read_data[an_cfg2,restart_an_txsm]=1'b1; 
	 p_sequencer.top_env.reg_write_anlt($sformatf("port%0d_%0s_csr",node,"an_cfg2"),read_data,speed);
	 `uvm_info(get_type_name(), $sformatf("configuring VIP agent config to restart AN:\n",p_sequencer.top_env.env_ip[inst].mac_cfg.print()), UVM_NONE);
	 p_sequencer.top_env.reset_vip(inst);
	 p_sequencer.top_env.reconfig_vip_for_an_mode(speed,inst);
	 #5us;
	 // FIX ME: Check AN status registers are reset to reset values
      end // for (int i=0;i<num_reset;i++)
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_restart_during_an_sequence_body";
...
endclass:an_restart_during_an_sequence
