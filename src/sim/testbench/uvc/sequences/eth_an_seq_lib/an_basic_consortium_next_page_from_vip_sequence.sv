...
class an_basic_consortium_next_page_from_vip_sequence extends an_base_sequence;
   `uvm_object_utils(an_basic_consortium_next_page_from_vip_sequence)
   
   function new(string name = "an_basic_consortium_next_page_from_vip_sequence");
      super.new(name);
...
`endif
  ...
  `endif
   ...
   task automatic an_basic_consortium_np(speed_e speed, int node, int inst);
      ...
      if((p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _50G && p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.ch_num == 1) ||
         ...
         p_sequencer.top_env.vip_dme_page_cfg(speed,inst,vip_np_en,vip_np_num,IEEE);
      ...
      fork : wait_next_page
         begin
           ...
           `uvm_error(get_type_name(), $sformatf("%s: Base page received from VIP incorrect.\nEXP selector=%0x, ACT selector=%0x \nEXP enonce=%0x, ACT enonce=%0x, \nEXP pause=%0x, ACT pause=%0x, \nEXP rf=%0x, ACT rf=%0x, \nEXP np=%0x, ACT np=%0x, \nEXP tnonce=%0x, ACT tnonce=%0x",func_name,vip_base_page_sent[4:0],c7_data[4:0],vip_base_page_sent[9:5],c7_data[9:5],vip_base_page_sent[12:10],c7_data[12:10],vip_base_page_sent[13],c7_data[13],vip_base_page_sent[15],c7_data[15],vip_base_page_sent[20:16],c8_data[4:0]));
         end
       	 begin
	    ...
	    if(read_data != exp_read_data) // C2.1 page received bit is clear on read 
	      `uvm_error(get_type_name(), $sformatf("%s: AN  STATUS read data is incorrect. EXP=%0x, ACT=%0x",func_name,exp_read_data,read_data));
	    /*p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_status6"),read_data,speed,.disable_check(1'b1)); HSD::16013345640 //The entire register 0xCE is reserved 
	 ...
	 begin
	    forever begin
	       ...
	       vip_next_page_sent=p_sequencer.top_env.env_ip[inst].m_snps_eth_pcs66_agent.vif.debug_bus.debug_bus_an.debug_bus_an73_bfm.arbiter_48b_linkword_op;
              ...
              /* if (vip_next_page_sent[15:0]!=c9_data[15:0] || vip_next_page_sent[47:16]!=ca_data[31:0]) HSD:: 16013311079 //Deferred till 21.3
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_basic_consortium_next_page_from_vip_sequence_body";
...
endclass:an_basic_consortium_next_page_from_vip_sequence
