...
class an_main_reset_during_an_sequence extends an_base_sequence;
   `uvm_object_utils(an_main_reset_during_an_sequence)
     int num_reset;
     int num_pages_sent;
     bit vip_np=0;

      
   function new(string name = "an_main_reset_during_an_sequence");
      super.new(name);
...
`endif
  ...
  `ifdef UVM_VERSION_1_1
   virtual task pre_start();
      ...
      an_ovrd_param=$urandom_range(0,1);
      super.pre_start();
   endtask:pre_start
  `endif   
   
   task automatic an_main_reset(speed_e speed, int node, int inst);
      string func_name = "an_main_reset_task";
	 ...
	 fork : wait_an
	    begin
	       ...
	       if (!vip_np)
		 num_pages_sent = $urandom_range(1,7);
	       else
		 num_pages_sent = $urandom_range(8,12);
	...
	// wait_an_tx_disable(inst);
     if (an_bp_ctrl == 1 || an_ovrd_param == 1) begin
    `uvm_info(get_type_name(), $sformatf("%s: base page or override bit is set ",func_name), UVM_LOW);
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "an_main_reset_during_an_sequence_body";
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 10G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_10g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 25G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_25g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 40G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_40g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 50G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_50g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 100G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_100g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 200G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_200g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 400G - an_main_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_400g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]), UVM_NONE);
                     an_main_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                     p_sequencer.top_env.wait_rx_pcs_ready(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i,anlt_std);
                     fork
                       send_eth_frame(DATA_FRAME,ETH_VIP_AVL_RX,10,i);  
...
endclass:an_main_reset_during_an_sequence
