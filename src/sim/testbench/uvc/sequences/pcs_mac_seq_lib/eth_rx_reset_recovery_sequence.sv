...
class eth_rx_reset_recovery_sequence extends eth_base_sequence;
   uvm_event_pool a_event_pool;
   uvm_event assertion_event;
   bit [2:0] rst_sig;

   `uvm_object_utils(eth_rx_reset_recovery_sequence)
      
     function new(string name = "eth_rx_reset_recovery_sequence");
	super.new(name);
...
`endif
      ...
      `uvm_info("eth_rx_reset_recovery_sequence", "Executing eth_rx_reset_recovery_sequence ...", UVM_LOW)
...
`endif
	    ...
	    fork 
	       begin
		  `uvm_info("eth_rx_reset_recovery_sequence", $sformatf("3 : rst_sig := %0p",rst_sig),UVM_NONE)
	            p_sequencer.env.apply_reset("hard",0,1,0,$urandom_range(21,50));
		  reconfig_for_an(rst_sig);
		  p_sequencer.env.wait_for_linkup(.tx_sync(0),.rx_sync(1),.ip_sync(0));
	       end  
	       begin
		  //#200ns;
...
`endif 
	 ...
	 begin
      ...
      `uvm_info("eth_rx_reset_recovery_sequence", "Exiting eth_avalonst_to_serial_simplex_sequence ...", UVM_LOW)
	endtask
