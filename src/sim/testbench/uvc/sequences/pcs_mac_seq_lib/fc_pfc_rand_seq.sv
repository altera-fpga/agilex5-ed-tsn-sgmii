...
class fc_pfc_rand_seq extends eth_base_sequence;
  fc_pause_sequence pause_seq;
  `uvm_object_utils(fc_pfc_rand_seq)
...
function new(string name = "seq_0");
  ...
  `uvm_info("eth_seq_lib", "running fc_pfc_rand sequence\n",UVM_LOW)
    pause_seq.fc_mode=reg_mode;
    pause_seq.pause_pfc=1;
  ...
  if(rd_data[0]!=0)   `uvm_error("fc_pfc_rand_seq", $sformatf("REGISTERS_tx_xof_en_tx_pause_qnumber_OFFSET_REG bit 0 value must be 0")); // As per FB 586418 ,this register will always have value 0.
    p_sequencer.env.reg_read(`GET_REG_ADDR(mac_cfg_rx_pause_fwd_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),rd_data,1);
  if(rd_data[0]!=p_sequencer.env.dyn_rcfg_obj_inst.rx_fc_fwd)   `uvm_error("fc_pfc_rand_sequence", $sformatf("REGISTERS_rx_pause_fwd_OFFSET_REG value must be initialized as per parameter"));
...
`uvm_info("fc_pfc_rand_seq", "=========================FLOW CONTROL INFO FOR THIS SIMULATION===================\n",UVM_LOW)
`uvm_info("fc_pfc_rand_seq", "=========================TX PATH===================\n",UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("Ports Enabled for input:%h...\n",port_en),UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("Ports Enabled for sending automatic holdoff xoff frames:%h...\n",holdoff_en),UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("SFC pause quanta:%h...\n",sfc_pause_quanta),UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("SFC hold quanta:%h...\n",sfc_holdoff_quanta),UVM_LOW)
    if(ready_drop) `uvm_info("fc_pfc_rand_seq","SFC will inhibit traffic flow...\n",UVM_LOW)
    else `uvm_info("fc_pfc_rand_seq","SFC will not stop TX traffic...\n",UVM_LOW)
    if(same_holdoff_en) `uvm_info("fc_pfc_rand_seq",$sformatf("all PFC queues will have same holdoff value which is:%h",same_holdoff_quanta),UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("TX SOURCE ADDR:%h...\n",tx_sa),UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("TX DEST ADDR:%h...\n",tx_da),UVM_LOW)
    if(tx_fc_en[1]) `uvm_info("fc_pfc_rand_seq","PFC frames will be passed by DUT...\n",UVM_LOW)
    if(tx_fc_en[0]) `uvm_info("fc_pfc_rand_seq","SFC frames will be passed by DUT...\n",UVM_LOW)
`uvm_info("fc_pfc_rand_seq", "=========================RX PATH===================\n",UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("Ports Enabled for output:%h...\n",rx_pfc_en),UVM_LOW)
    if(rx_fc_fwd) `uvm_info("fc_pfc_rand_seq","Control frames will be forwarded on rx user interface...\n",UVM_LOW)
    else `uvm_info("fc_pfc_rand_seq","Control frames will not be forwarded on rx user interface...\n",UVM_LOW)
    `uvm_info("fc_pfc_rand_seq",$sformatf("RX DEST ADDR in VIP mode:%h...\n",rx_da),UVM_LOW)
    if(rx_fc_en[1]) `uvm_info("fc_pfc_rand_seq","PFC frames will be passed by DUT...\n",UVM_LOW)
    if(rx_fc_en[0]) `uvm_info("fc_pfc_rand_seq","SFC frames will be passed by DUT...\n",UVM_LOW)
 ...
 //`endif
   ...
   `uvm_info("fc_pfc_rand_seq", "All fc registers are written, triggered event..\n",UVM_LOW)

   `uvm_info("fc_pfc_rand_seq", "This test will run in PFC Mode...\n",UVM_LOW)
     for(int i=0;i<8;i++) begin
      `uvm_info("fc_pfc_rand_seq",$sformatf("PAUSE TIME[%d]=%dns\n",i,12.8*quanta[i]),UVM_LOW)
      `uvm_info("fc_pfc_rand_seq",$sformatf("HOLD TIME[%d]=%dns\n",i,12.8*hold_quanta[i]),UVM_LOW)
    end 
