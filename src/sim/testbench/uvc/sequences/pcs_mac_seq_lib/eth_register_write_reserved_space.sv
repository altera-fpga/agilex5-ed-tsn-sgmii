...
class eth_register_write_reserved_space extends eth_base_sequence;
  ...
  `uvm_object_utils(eth_register_write_reserved_space)

  function new(string name = "eth_register_write_reserved_space");
    super.new(name);
...
// FIXME-MISSING_REG_IN_GDR        //  regs[i].get_address() == `REGISTERS_phy_pma_sloop_OFFSET_REG ||    
    ...
    foreach(regs[i]) begin
      `uvm_info("eth_register_write_reserved_space", $sformatf("Register compare_disable[('h%h)-(%s)] = %0d",regs[i].get_address(),regs[i].get_name(),compare_disable[regs[i].get_address()]), UVM_MEDIUM)
    end

    `uvm_info("eth_register_write_reserved_space", "1:Read registers", UVM_NONE)
    foreach(regs[i]) 
    begin
      p_sequencer.env.reg_read(regs[i].get_address(),read_data,compare_disable[regs[i].get_address()]);
    end
    
    `uvm_info("eth_register_write_reserved_space", "2:Read registers", UVM_NONE)
    foreach(regs[i]) 
    begin
      p_sequencer.env.reg_read(regs[i].get_address(),read_data,compare_disable[regs[i].get_address()]);
    end

    `uvm_info("eth_register_write_reserved_space", "3:Read registers", UVM_NONE)
    foreach(regs[i]) 
    begin
      p_sequencer.env.reg_read(regs[i].get_address(),read_data,compare_disable[regs[i].get_address()]);
    end
    
    `uvm_info("eth_register_write_reserved_space", " write Pattern on reserved space('h300-'h309)...", UVM_LOW)
    for(int addr = 'h300;addr < 'h310;addr++)
    begin
      p_sequencer.env.reg_write(addr,'h55555555);
      p_sequencer.env.reg_read(addr,read_data);
    end
     
    `uvm_info("eth_register_write_reserved_space", " write Pattern on reserved space('h311-'h9FF)...", UVM_LOW)
    for(int addr = 'h311;addr <= 'h9FF;addr++)
    begin
      if (addr == 'h340 || addr == 'h341 || addr == 'h342 || addr == 'h32b)//pbenittx:skipping `h32b register since it is not present in crete_3 register set.
    `uvm_info("eth_register_write_reserved_space", "addr is 340 or 341 or 342:fb", UVM_NONE) //fb:551427 these registers need to be check in last
       else
      begin
      p_sequencer.env.reg_write(addr,'h55555555);
      if((addr > 'h839 && addr <= 'h8FF) || (addr > 'h939 && addr <= 'h9FF) || addr == 'h315 || addr == 'h329 || addr == 'h320 || addr == 'h313 || addr == 'h32a || (addr >= 'h361 && addr <= 'h374) ||  addr == 'h37c ||addr == 'h321 || addr == 'h323 || addr == 'h326 || addr == 'h328 || (addr >= 'h330 && addr <= 'h333) || (addr >= 'h37F && addr <= 'h382)) 
