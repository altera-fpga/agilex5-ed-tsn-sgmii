...
class fb_495115_hard_rst_seq extends eth_base_sequence;
  bit[2:0] hard_rst_sig;
  bit[2:0] soft_rst_sig;
  bit[2:0] rst_sig;
  `uvm_object_utils(fb_495115_hard_rst_seq)
  ...
  virtual task body();
    `uvm_info("body", "started fb_495115_hard_rst_seq ...", UVM_NONE)
   ...
   for (int i=0;i<7;i++) begin
    ...
    `endif
     `uvm_info("fb_495115_hard_rst_seq", "5. Apply eio_sys_rst,csr reset or set CTRL_CONFIG register bit 0 to clear the stats counter", UVM_NONE)
     //hard_rst
     hard_rst_sig++;
     `uvm_info("apply_hard_reset", $sformatf("hard_reset := %0d | hard_rst_combination := %0d",i,hard_rst_sig),UVM_NONE)
     p_sequencer.env.apply_reset("hard",hard_rst_sig[2],hard_rst_sig[1],hard_rst_sig[0],$urandom_range(21,50));
     reconfig_for_an(hard_rst_sig);
    ...
    `endif  
     p_sequencer.env.wait_for_linkup(.tx_sync(hard_rst_sig[2]),.rx_sync(hard_rst_sig[1]),.ip_sync(hard_rst_sig[0]));
    ...
    `uvm_info("body", "ended fb_495115_hard_rst_seq ...", UVM_NONE)
  endtask
endclass : fb_495115_hard_rst_seq
