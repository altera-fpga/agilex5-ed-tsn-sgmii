...
class eth_phy_deskew_reg_sequence extends eth_base_sequence;
  uvm_reg_data_t read_data;

  `uvm_object_utils(eth_phy_deskew_reg_sequence)
  ...
  virtual task body();
     ...
     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     if(read_data != 'h0) `uvm_error("eth_phy_deskew_reg_sequence", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));

     p_sequencer.env.wait_for_linkup(.tx_sync(0),.rx_sync(0),.ip_sync(1));

     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     //if(read_data != 'h1) `uvm_error("eth_phy_deskew_reg_sequence", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
     ...
     ////if(read_data != 'h3) `uvm_error("eth_phy_deskew_reg_sequence", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));

     //p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_cfg_phy_ehip_pcs_modes_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data); //sclr_frame_err bit found in this GDR register bit:12
     //p_sequencer.env.reg_write(`GET_REG_ADDR(ehip_cfg_phy_ehip_pcs_modes_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),{read_data[31:13],1'b1,read_data[11:0]});

     //p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     //if(read_data[1] != 'h0) `uvm_error("eth_phy_deskew_reg_sequence", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
