...
class eth_phy_deskew_reg_sequence_FB_509441 extends eth_base_sequence;
  uvm_reg_data_t read_data;

  `uvm_object_utils(eth_phy_deskew_reg_sequence_FB_509441)

  function new(string name = "eth_phy_deskew_reg_sequence_FB_509441");
    super.new(name);
  ...
  virtual task body();
     ...
     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     if(read_data != 'h0) `uvm_error("eth_phy_deskew_reg_sequence_FB_509441", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));

     p_sequencer.env.wait_for_linkup(.tx_sync(0),.rx_sync(0),.ip_sync(1));

     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     // comparison value changed according to HSD:1607575739
     if(read_data != 'h3) `uvm_error("eth_phy_deskew_reg_sequence_FB_509441", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
     ...
     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     //This checker needs clarifcation from faishal as it may deskew or may not in short period.
     if(read_data != 'h3) `uvm_error("eth_phy_deskew_reg_sequence_FB_509441", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
     ...
     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     if(read_data[1] != 'h0) `uvm_error("eth_phy_deskew_reg_sequence_FB_509441", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
