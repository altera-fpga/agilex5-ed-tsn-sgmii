...
class eth_fc_cov_seq extends eth_base_sequence;
  fc_pause_sequence pause_seq;
  `uvm_object_utils(eth_fc_cov_seq)
...
function new(string name = "seq_0");
  ...
  virtual task body();
   ...
   pause_mode[8] = 0; //0 - port based, 1 - reg based //$uranodm();

  `uvm_info("eth_seq_lib", "running fc_rand sequence\n",UVM_LOW)
   pause_seq.fc_mode=pause_mode;
   ...
   `uvm_info("eth_fc_cov_seq", "All fc registers are written, triggered event..\n",UVM_LOW)

   `uvm_info("eth_fc_cov_seq", "This test will run in Pause Mode...\n",UVM_LOW)
   `uvm_info("eth_fc_cov_seq",$sformatf("PAUSE TIME=%dns\n",12.8*sfc_pause_quanta),UVM_LOW)
   `uvm_info("eth_fc_cov_seq",$sformatf("HOLD TIME=%dns\n",12.8*sfc_holdoff_quanta),UVM_LOW)
   ...
   for(int i=0;i<2;i++) begin
     // Not randomizing other bits as PFC is not supported for DM
     pause_mode[8] = $urandom_range(1,0); //0 - port based, 1 - reg based //$uranodm();
     pause_seq.fc_mode   = pause_mode; //register or port
     pause_seq.pause_pfc=0;

     std::randomize(tx_pausefrm_en)with{tx_pausefrm_en dist{0:/5,1:/5};}; 
     p_sequencer.env.reg_read(`GET_REG_ADDR(tx_pauseframe_enable_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),rd_data,1);
     rd_data[0]   = tx_pausefrm_en;//1; //enable tx pause frame transmission 
     rd_data[2:1] = {1'b0,pause_seq.fc_mode[8]};
     ...
     if(i==0) wait_fc_reg_write.trigger();
     $display("Iteration :%0d",i);
     display_settings();

     fork
        pause_seq.start(p_sequencer.fc_sqr);
	send_eth_frame_with_fix_size(.eth_frame(DATA_FRAME),.frame_size($urandom_range(100,200)),.no_of_frame(5),.path(AVL_TX_ETH_VIP));
        begin
  ...
  `else
    fork
      pause_seq.start(p_sequencer.fc_sqr);
      send_eth_frame_with_fix_size(.eth_frame(DATA_FRAME),.frame_size($urandom_range(100,200)),.no_of_frame(50),.path(AVL_TX_ETH_VIP));
    join
  `endif
    #500ns;
 endtask
   ...
   function void display_settings();
   	`uvm_info("eth_fc_cov_seq", "=========================FLOW CONTROL INFO ===================\n",UVM_LOW)
   	`uvm_info("eth_fc_cov_seq", "=========================TX PATH===================\n",UVM_LOW)
   	`uvm_info("eth_fc_cov_seq",$sformatf("SFC pause quanta:%h...\n",sfc_pause_quanta),UVM_LOW)
   	`uvm_info("eth_fc_cov_seq",$sformatf("SFC hold quanta:%h...\n",sfc_holdoff_quanta),UVM_LOW)
   	if(ready_drop) `uvm_info("eth_fc_cov_seq","SFC will inhibit traffic flow...\n",UVM_LOW)
   	else `uvm_info("eth_fc_cov_seq","SFC will not stop TX traffic...\n",UVM_LOW)
   	`uvm_info("eth_fc_cov_seq", "=========================RX PATH===================\n",UVM_LOW)
   	`uvm_info("eth_fc_cov_seq",$sformatf("Ports Enabled for output:%h...\n",rx_pfc_en),UVM_LOW)
   	if(rx_fc_fwd) `uvm_info("eth_fc_cov_seq","Control frames will be forwarded on rx user interface...\n",UVM_LOW)
   	else `uvm_info("eth_fc_cov_seq","Control frames will not be forwarded on rx user interface...\n",UVM_LOW)
   	`uvm_info("eth_fc_cov_seq",$sformatf("RX DEST ADDR in VIP mode:%h...\n",rx_da),UVM_LOW)
   	if(rx_fc_en[1]) `uvm_info("eth_fc_cov_seq","PFC RX frames will be passed by DUT...\n",UVM_LOW)
   	if(rx_fc_en[0]) `uvm_info("eth_fc_cov_seq","SFC RX frames will be passed by DUT...\n",UVM_LOW)
