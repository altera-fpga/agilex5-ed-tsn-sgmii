...
// sequence_name :eth_register_access_sequence_3
// 1. Apply IP reset (maintained by testcase) 
// 2. Performing walk 1 pattern in all the registers sequentially.

class eth_register_access_sequence_3 extends eth_base_sequence;
  uvm_reg_data_t read_data;
  uvm_reg 	   regs[$];
  bit [1:0] compare_disable[integer];
  rand int k;

  `uvm_object_utils(eth_register_access_sequence_3)

  function new(string name = "eth_register_access_sequence_3");
    super.new(name);
  ...
  virtual task body();
    ...
    `uvm_info(get_type_name(), "started eth_register_access_sequence_3 ...", UVM_NONE)
    ...
    foreach(regs[i]) begin
      `uvm_info("eth_register_access_sequence_3", $sformatf("Register compare_disable[('h%h)-(%s)] = %0d",regs[i].get_address(),regs[i].get_name(),compare_disable[regs[i].get_address()]), UVM_NONE)
    end
    
    // 2. Performing walk 1 pattern.
    randomize(k) with {k inside {0,4,8,12,16,20,24,28};};
    `uvm_info("eth_register_access_sequence_3", "Performing walk 1 Pattern...", UVM_LOW)
    foreach(regs[i]) 
    begin
      for(int j=k;j<=k+3;j++) 
      begin
    ...
    `uvm_info(get_type_name(), "finished eth_register_access_sequence_3 ...", UVM_NONE)
  endtask
endclass
