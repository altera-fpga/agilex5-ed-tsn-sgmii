...
class eth_phy_deskew_reg_sequence_1 extends eth_base_sequence;
  uvm_reg_data_t read_data;

  `uvm_object_utils(eth_phy_deskew_reg_sequence_1)

  function new(string name = "eth_phy_deskew_reg_sequence_1");
    super.new(name);
  ...
  virtual task body();
     ...
     p_sequencer.env.reg_read(`GET_REG_ADDR(ehip_stats_lanes_deskewed_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),read_data,1);
     //if(read_data != 'h1) `uvm_error("eth_phy_deskew_reg_sequence_1", $sformatf("REGISTERS_LANE_DESKEWED_OFFSET_REG value is not correct"));
