...
class eth_register_timeout extends eth_base_sequence;
  ...
  `uvm_object_utils(eth_register_timeout)

  function new(string name = "eth_register_timeout");
    super.new(name);
  ...
  virtual task body();
    ...
    `uvm_info(get_type_name(), "started eth_register_timeout ...", UVM_NONE)
    ...
    foreach(regs[i]) begin
      `uvm_info("eth_register_timeout", $sformatf("Register compare_disable[('h%h)-(%s)] = %0d",regs[i].get_address(),regs[i].get_name(),compare_disable[regs[i].get_address()]), UVM_NONE)
    end
    
    // 2. Disabled the clock for MAC and Lphy
    `uvm_info("eth_register_timeout", "Disabled the clock for MAC and Lphy...", UVM_LOW)
    ...
    `uvm_info("eth_register_timeout", "Performing Read on MAC and FEC Stat register when clock is disabled... ", UVM_LOW)
    ...
    // TBD Venkatkx need to update for all the speeds
     `uvm_info("eth_register_timeout", "Performing read write Pattern on reserved space... ", UVM_LOW)
    ...
    `uvm_info(get_type_name(), "finished eth_register_timeout ...", UVM_NONE)
  endtask
endclass
