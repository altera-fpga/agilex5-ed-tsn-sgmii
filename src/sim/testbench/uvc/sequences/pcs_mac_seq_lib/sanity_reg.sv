...
class sanity_reg extends eth_base_sequence;

  `uvm_object_utils(sanity_reg)
  ...
  //  bit [31:0] data_read;
   ...
   p_sequencer.env.reg_read_axi('h4, 'h1FC2_0300, data_read);
   if(data_read != 'h7)
      `uvm_error(get_name(), $psprintf("reset (csr) = %0h expected = 'h7", data_read))
   p_sequencer.env.reg_write_axi('h0, 'h1FC2_0280, 32'hffffffff);
   p_sequencer.env.reg_read_axi('h0, 'h1FC2_0280, data_read);
   if(data_read != 'h0)
