...
class eth_register_write_reserved_space_2 extends eth_base_sequence;
  uvm_reg_data_t read_data;
  uvm_reg 	regs_org[$],regs[$];
  bit [2:0]     reset_sel; 
  bit compare_disable[integer];
  bit [31:0] max_address = 'h7FFF;
  bit [31:0] min_address = 'hF00;
  int reg_index[$];

  `uvm_object_utils(eth_register_write_reserved_space_2)

  function new(string name = "eth_register_write_reserved_space_2");
    super.new(name);
  ...
  virtual task body();
    ...
    foreach(regs[i]) begin
      `uvm_info("eth_register_write_reserved_space_2", $sformatf("Register compare_disable[('h%h)-(%s)] = %0d",regs[i].get_address(),regs[i].get_name(),compare_disable[regs[i].get_address()]), UVM_MEDIUM)
    end
 

    `uvm_info("eth_register_write_reserved_space_2", "write Pattern on reserved space('hF00-'h3FFF)...", UVM_LOW)
    for(int addr ='hF00;addr <= 'h3FFF;addr++)
    begin
      p_sequencer.env.reg_write(addr,'hAAAAAAAA);
      p_sequencer.env.reg_read(addr,read_data,1);
    end

   `uvm_info("eth_register_write_reserved_space_2", "write Pattern on reserved space('h4000-'h7FFF)...", UVM_LOW)
    for(int addr ='h4000;addr <= 'h7FFF;addr++)
    begin
      p_sequencer.env.reg_write(addr,'hAAAAAAAA);
      p_sequencer.env.reg_read(addr,read_data,1);
    end
