...
class eth_b2b_hard_reset_recovery_short_frames_sequence extends eth_base_sequence;
  uvm_event_pool a_event_pool;
  uvm_event assertion_event;
  bit [2:0] rst_sig,rst_sig_1,rst_sig_2;
  int loop_cnt;

  `uvm_object_utils(eth_b2b_hard_reset_recovery_short_frames_sequence)
  
  function new(string name = "eth_b2b_hard_reset_recovery_short_frames_sequence");
    super.new(name);
  ...
  virtual task body();
   ...
   //`ifdef ANLT   //dsamantx:FIXME for GDR_ANLT
   //uvm_hdl_force("eth_env_top.avst_tx_rtb.source.u.u_bfm.response_timeout",300000);
   //`endif

   `uvm_info("eth_b2b_hard_reset_recovery_short_frames_sequence", "Executing eth_b2b_hard_reset_recovery_short_frames_sequence ...", UVM_LOW)
   ...
   repeat(2) begin
     fork : reset_thread_1
       begin
           p_tx = process :: self();
	   repeat(50) send_eth_frame_with_fix_size(RANDOM_FRAME,$urandom_range(64,128),1,AVL_TX_ETH_VIP);
       end
       begin
           p_rx = process :: self();
	   repeat(50) send_eth_frame_with_fix_size(RANDOM_FRAME,$urandom_range(64,128),1,ETH_VIP_AVL_RX);
       end
       begin
         ...
         fork 
           begin
             ...
             for(int i=0;i<loop_cnt;i++) begin
               ...
               `uvm_info("eth_b2b_hard_reset_recovery_short_frames_sequence", $sformatf("loop_cnt %0d : rst_sig_1 := %0p",i,rst_sig_1),UVM_NONE)
    ...
    `endif 
     ...
     fork
       begin
	 repeat(25) send_eth_frame_with_fix_size(RANDOM_FRAME,$urandom_range(64,128),1,AVL_TX_ETH_VIP);
       end
    
       begin
	 repeat(25) send_eth_frame_with_fix_size(RANDOM_FRAME,$urandom_range(64,128),1,ETH_VIP_AVL_RX);
       end
     join
   ...
   `uvm_info("eth_b2b_hard_reset_recovery_short_frames_sequence", "Exiting eth_avalonst_to_serial_simplex_sequence ...", UVM_LOW)
  endtask
