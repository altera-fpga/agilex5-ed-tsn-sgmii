...
class eth_hard_reset_recovery_rst_delay_sequence extends eth_base_sequence;
  uvm_event_pool a_event_pool;
  uvm_event assertion_event;
  bit [2:0] rst_sig;

  `uvm_object_utils(eth_hard_reset_recovery_rst_delay_sequence)
  
  function new(string name = "eth_hard_reset_recovery_rst_delay_sequence");
    super.new(name);
  ...
  virtual task body();
   ...
   `uvm_info("eth_hard_reset_recovery_rst_delay_sequence", "Executing eth_hard_reset_recovery_rst_delay_sequence ...", UVM_LOW)
   ...
   fork : reset_thread_1
	...
	`endif
       ...
       fork 
         begin
           `uvm_info("eth_hard_reset_recovery_rst_delay_sequence", $sformatf("3 : rst_sig := %0p",rst_sig),UVM_NONE)
           p_sequencer.env.apply_reset("hard",rst_sig[2],rst_sig[1],rst_sig[0],$urandom_range(21,50),$urandom_range(5,10),$urandom_range(5,10),$urandom_range(5,10));
  ...
  `endif 
   ...
   `uvm_info("eth_hard_reset_recovery_rst_delay_sequence", "Exiting eth_avalonst_to_serial_simplex_sequence ...", UVM_LOW)
  endtask
