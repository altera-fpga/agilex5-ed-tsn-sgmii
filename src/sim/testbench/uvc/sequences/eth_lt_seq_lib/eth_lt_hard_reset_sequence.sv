...
class lt_hard_reset_sequence extends eth_lt_base_sequence;

  bit [2:0] reset_bits;
  bit lt_in_middle;
  int reset_point_stage;

  `uvm_object_utils(lt_hard_reset_sequence)

  function new(string name = "lt_hard_reset_sequence");
    super.new(name);
...
task lt_hard_reset(speed_e speed, int node, int inst);
 ...
 //       regs = p_sequencer.top_env.env_ip[inst].reg_model.default_map.get_reg_by_offset($sformatf("port%0d_%0s_csr",node,"anlt_seq_cfg,speed,node));
 //   read_data[0] = 1'b0;
 //   regs.predict(.value(read_data),.kind(UVM_PREDICT_DIRECT), .map( p_sequencer.top_env.env_ip[inst].reg_model.default_map));
    ...
    reset_point_stage = $urandom_range(1,'h3F);
    lt_in_middle = $urandom_range(0,1);
     ...
     reset_bits = $urandom_range (1,3);
     p_sequencer.top_env.env_ip[inst].apply_reset(.rst_type("hard"),.ip_rst(1));
     //apply_hard_reset(reset_bits[0],reset_bits[1],0);//tx & rx reset should not affect LT and AN
    // p_sequencer.top_env.env_ip[inst].apply_reset(.rst_type("hard"),.tx_rst(reset_bits[0]),.rx_rst(reset_bits[1]));

     case({AN_en,LT_en})
             'b01:  begin 
                      //      reg_predict_read($sformatf("%s","an_status"),speed,node,'h000030); 
                      //      reg_predict_read($sformatf("%s","an_status5"),speed,node,'h0);
		     if(lt_in_middle == 1'b0) begin
                if ((p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _100G) && (p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.ch_num == 2)) begin
                 reg_predict_read($sformatf("%s","lt_status1"),speed,node,'h00001111);
                end
                if ((p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _100G) && (p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.ch_num == 1)) begin
               ...
               if(p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _400G) begin
                reg_predict_read($sformatf("%s","lt_status1"),speed,node,'h11111111);
                            ...
                            if(read_data == 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(lt_status1,speed,node) data failed"); 
                               reg_predict_read($sformatf("%s","seq_status"),speed,node,'h200);
		     end
   ...
   case({AN_en,LT_en})
     ...
     'b10: begin 
             ...
             if ((p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _50G) && (p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.ch_num == 2)) begin
              //reg_predict_read($sformatf("%s","lt_status1"),speed,node,'h00000101);
             end
             p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"lt_status1"),read_data,speed);
             p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_status1"),read_data,speed,1);
             if(read_data == 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(an_status1,speed,node) data failed"); 
...
//   p_sequencer.top_env.reconfig_vip_for_datamode(speed,node);
   wait_for_seq_mode(speed,inst);
        ...
        //if(p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _25G) begin
        //`uvm_info(get_name(), $sformatf("Waiting for data mode transiton  "), UVM_NONE);
        //wait(p_sequencer.top_env.env_ip[inst].spy_if.seq_mode[3] ==1);
        //end

        //if(p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _10G) begin
        //`uvm_info(get_name(), $sformatf("Waiting for data mode transiton  "), UVM_NONE);
        //wait(p_sequencer.top_env.env_ip[inst].spy_if.seq_mode[2] ==1);
        //end

    `uvm_info("wait_for_lt_complete", $sformatf("Waiting for seq link READY to be up "), UVM_NONE);
           wait(p_sequencer.top_env.env_ip[inst].spy_if.seq_link_ready == 1'b1);
    `uvm_info("wait_for_lt_complete", $sformatf("Waiting for seq link READY Done "), UVM_NONE);
    ...
    `uvm_info(get_name(), $sformatf("******* wait_for_rx_pcs_ready start ***************"), UVM_NONE);
...
endtask
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "lt_hard_reset_sequence_body";
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 10G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_10g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 25G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_25g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 40G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_40g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 50G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_50g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 100G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_100g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 200G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_200g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 400G - lt_hard_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_400g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]), UVM_NONE);
                     lt_hard_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
...
endclass:lt_hard_reset_sequence
