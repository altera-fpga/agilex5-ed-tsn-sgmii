// (C) 2001-2023 Intel Corporation. All rights reserved.
// Your use of Intel Corporation's design tools, logic functions and other 
// software and tools, and its AMPP partner logic functions, and any output 
// files from any of the foregoing (including device programming or simulation 
// files), and any associated documentation or information are expressly subject 
// to the terms and conditions of the Intel Program License Subscription 
// Agreement, Intel FPGA IP License Agreement, or other applicable 
// license agreement, including, without limitation, that your use is for the 
// sole purpose of programming logic devices manufactured by Intel and sold by 
// Intel or its authorized distributors.  Please refer to the applicable 
// agreement for further details.


class eth_lt_remote_rx_ready_sequence extends eth_lt_base_sequence;
 
  bit LT_en;
  bit AN_en;
  bit disable_max_timer;
  bit [4:0] mwt_multiplier;
  bit lt_failure_response; 
  bit disable_link_ready_cancel;; 
  bit [7:0] restart_status[4];
  bit err_flag;
  bit [3:0] Lane_select_err;
  `uvm_object_utils(eth_lt_remote_rx_ready_sequence)

  function new(string name = "eth_lt_remote_rx_ready_sequence");
    super.new(name);
	`ifdef UVM_POST_VERSION_1_1
     set_automatic_phase_objection(1);
    `endif
 endfunction:new

  virtual task body();
  if(p_sequencer.env.dyn_rcfg_obj_inst.speed == _100G) begin
   
   p_sequencer.env.apply_reset(.rst_type("hard"),.ip_rst(1),.reset_period(11));

   Lane_select_err = $urandom_range(1,'hF);
   p_sequencer.env.mac_callback.local_status_reg[15] = 0 ;
   bringup_status_read();

   prbs_select();

   LT_en = 1;
   AN_en = $urandom_range(0,1);
   enable_disable_lt(LT_en);
   enable_disable_an(AN_en);
   reset_sequencer();

   disable_max_timer = 0;
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_lt_cfg1_OFFSET_REG,read_data);
   read_data[1] = disable_max_timer;
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_write(`REGISTERS_lt_cfg1_OFFSET_REG,read_data);

   mwt_multiplier  = 'h0; 
   disable_link_ready_cancel = 1; 
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_lt_params_OFFSET_REG,read_data);
   read_data[9]   = disable_link_ready_cancel;
   read_data[4:0] = mwt_multiplier;
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_write(`REGISTERS_lt_params_OFFSET_REG,read_data);
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_lt_params_OFFSET_REG,read_data);

   case({AN_en,LT_en})
// FIXME-MISSING_REG_IN_GDR     'b00:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h2000);
// FIXME-MISSING_REG_IN_GDR     'b01:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h200);
// FIXME-MISSING_REG_IN_GDR     'b10:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h100);
// FIXME-MISSING_REG_IN_GDR     'b11:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h100);
   endcase

// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_write(`REGISTERS_anlt_seq_cfg_OFFSET_REG,(($urandom & 'h0000_7007) | 'h0000_1004)); 
   #1us;
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_anlt_seq_cfg_OFFSET_REG,read_data);
   lt_failure_response =  read_data[12];
   `uvm_info(get_name(), $sformatf("******* ERROR INJECTION ***************"), UVM_NONE);
   `uvm_info(get_name(), $sformatf("LT_en                   :%0d ",LT_en), UVM_NONE);
   `uvm_info(get_name(), $sformatf("AN_en                   :%0d ",AN_en), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Restart AN sequencer    :%0d ",read_data[0]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Disabl AN Timer         :%0d ",read_data[1]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Disable LF Timer        :%0d ",read_data[2]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("LT Failure Response     :%0d ",read_data[12]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("LT Fail if Hiber on/off :%0d ",read_data[13]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Skip LT on AN Timeout   :%0d ",read_data[14]), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Disable max wait timer  :%0d ",disable_max_timer), UVM_NONE);
   `uvm_info(get_name(), $sformatf("disable_link_ready_cancel:%0d ",disable_link_ready_cancel), UVM_NONE);
   `uvm_info(get_name(), $sformatf("p_sequencer.env.mac_callback.enable_lt_status_reg_err            :%0d ",p_sequencer.env.mac_callback.enable_lt_status_reg_err), UVM_NONE);
   `uvm_info(get_name(), $sformatf("Lane_select_err                   :%0h ",Lane_select_err ), UVM_NONE);
   `uvm_info(get_name(), $sformatf("p_sequencer.env.mac_callback.local_status_reg               :%0d ",p_sequencer.env.mac_callback.local_status_reg), UVM_NONE);
   `uvm_info(get_name(), $sformatf("******* **************************"), UVM_NONE);


   wait_for_an_vip_event(AN_en);
   p_sequencer.env.reconfig_vip_for_lt_mode();

  fork :disable_rx_remote_ready_1
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 0 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[0][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 0 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[0].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 1 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[1][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 1 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[1].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 2 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[2][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 2 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[2].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 3 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[3][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 3 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[3].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   wait_for_lt_vip_event();
  end
  join

// FIXME-MISSING_REG_IN_GDR reg_predict_read(`REGISTERS_lt_status1_OFFSET_REG,'h01010101);
 p_sequencer.env.reconfig_vip_for_datamode();
 `uvm_info("wait_for_lt_complete", $sformatf("Waiting for RX PCS READY to be up "), UVM_NONE);
 wait(p_sequencer.env.master_agent.mast_agt_if.rx_pcs_ready==1);
 p_sequencer.env.enable_snps_errors();
 send_eth_frame(RANDOM_FRAME,ETH_VIP_MAC_BOTH,5);
// FIXME-MISSING_REG_IN_GDR   reg_predict_read(`REGISTERS_lt_status1_OFFSET_REG,'h01010101);
   
   LT_en = 1; 
   AN_en = $urandom_range(0,1);

   `uvm_info(get_name(), $sformatf("******* RTL CONFIG ***************"), UVM_NONE);
   `uvm_info(get_name(), $sformatf("LT_en                   :%0d ",LT_en), UVM_NONE);
   `uvm_info(get_name(), $sformatf("AN_en                   :%0d ",AN_en), UVM_NONE);
   `uvm_info(get_name(), $sformatf(" p_sequencer.env.mac_callback.lane_select                   :%0h ",p_sequencer.env.mac_callback.lane_select), UVM_NONE);
   `uvm_info(get_name(), $sformatf("******* **************************"), UVM_NONE);
   enable_disable_lt(LT_en);
   enable_disable_an(AN_en);
   reset_sequencer();
   p_sequencer.env.reset_vip();

   mwt_multiplier  = 'h0; 
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_lt_params_OFFSET_REG,read_data);
   read_data[9]   = ~disable_link_ready_cancel;
   read_data[4:0] = mwt_multiplier;
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_write(`REGISTERS_lt_params_OFFSET_REG,read_data);
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_lt_params_OFFSET_REG,read_data);


   if(AN_en) begin p_sequencer.env.reconfig_vip_for_an_mode(); end
     
   case({AN_en,LT_en})
// FIXME-MISSING_REG_IN_GDR     'b00:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h2000);
// FIXME-MISSING_REG_IN_GDR     'b01:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h200);
// FIXME-MISSING_REG_IN_GDR     'b10:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h100);
// FIXME-MISSING_REG_IN_GDR     'b11:reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h100);
   endcase

   wait_for_an_vip_event(AN_en);
   p_sequencer.env.reconfig_vip_for_lt_mode();
  
  fork :disable_rx_remote_ready_2
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 0 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[0][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 0 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[0].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end

  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 1 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[1][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 1 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[1].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 2 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[2][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 2 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[2].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end
  begin
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 3 start"), UVM_NONE);
   wait(p_sequencer.env.spy_if.lt_train_state[3][3:0] == 'h4); 
   `uvm_info("disable_rx_remote_ready_1", $sformatf("waiting for Link READY state Lane 3 complete"), UVM_NONE);
   uvm_hdl_force("eth_env_top.dut.top.GENKR.alt_ehipc2_kr_inst.kr_top.lane_gen[3].LT_GEN.LINK_TRAIN.TRAINING_SM.rmt_rx_ready",1'b0);
  end

  begin
     for(int i=0;i<(p_sequencer.env.spy_if.rtl_mwt) ;i++)
     begin
        `uvm_info(get_name(), $sformatf("#10us count =%0d ",i), UVM_NONE);
        #10us;
     end
     `uvm_info(get_name(), $sformatf(" disable_rx_remote_ready_2 err_flag: %0h ",err_flag), UVM_NONE);
     disable_lt_rx_checker();
     disable_lt_tx_checker();
    disable   disable_rx_remote_ready_2;
  end
  join
// FIXME-MISSING_REG_IN_GDR   reg_predict_read(`REGISTERS_lt_status1_OFFSET_REG,'h08080808);
   p_sequencer.env.reconfig_vip_for_datamode();
   wait(p_sequencer.env.master_agent.mast_agt_if.rx_pcs_ready==1);
   p_sequencer.env.enable_snps_errors();
   send_eth_frame(RANDOM_FRAME,ETH_VIP_MAC_BOTH,5);
// FIXME-MISSING_REG_IN_GDR   reg_predict_read(`REGISTERS_anlt_seq_status_OFFSET_REG,'h2001);
 end

   endtask
endclass
