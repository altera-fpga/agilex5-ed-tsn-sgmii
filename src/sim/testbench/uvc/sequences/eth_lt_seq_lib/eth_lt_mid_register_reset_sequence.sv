...
class lt_mid_register_reset_sequence extends eth_lt_base_sequence;
  ...
  `uvm_object_utils(lt_mid_register_reset_sequence)

  function new(string name = "lt_mid_register_reset_sequence");
    super.new(name);
 ...
 task lt_mid_register_reset(speed_e speed, int node, int inst);
     ...
     //    regs = p_sequencer.top_env.env_ip[inst].reg_model.default_map.get_reg_by_offset($sformatf("port%0d_%0s_csr",node,"seq_cfg"));
     //    read_data[0] = 1'b0;
     ...
     reset_point_stage = $urandom_range('h1,'h3F);

     if(p_sequencer.top_env.env_ip[inst].dyn_rcfg_obj_inst.speed == _100G) begin
     `uvm_info(get_name(), $sformatf(" reset_point_stage(%0h) complete for lt_in_middle %0d",reset_point_stage,lt_in_middle),UVM_NONE);
     `uvm_info(get_name(), $sformatf("waiting for training start :%0h ",p_sequencer.top_env.env_ip[inst].spy_if.lt_training),UVM_NONE);
      wait(p_sequencer.top_env.env_ip[inst].spy_if.lt_training !== 0 );
     `uvm_info(get_name(), $sformatf("training started :%0h ",p_sequencer.top_env.env_ip[inst].spy_if.lt_training),UVM_NONE);

     `uvm_info(get_name(), $sformatf("waiting for reset_point_stage(%0h)",reset_point_stage),UVM_NONE);
      wait({p_sequencer.top_env.env_ip[inst].spy_if.lt_trained,p_sequencer.top_env.env_ip[inst].spy_if.lt_frame_lock} > reset_point_stage );
     `uvm_info(get_name(), $sformatf(" {p_sequencer.top_env.env_ip[inst].spy_if.lt_trained,p_sequencer.top_env.env_ip[inst].spy_if.lt_frame_lock}=%0h",{p_sequencer.top_env.env_ip[inst].spy_if.lt_trained,p_sequencer.top_env.env_ip[inst].spy_if.lt_frame_lock}),UVM_NONE);
     end
     else
      begin
     ...
     read_data[0] = 0;
     reg_predict_read($sformatf("%s","an_cfg2"),speed,node,read_data,speed); 
   ...
   disable_lt_tx_checker(inst);
     ...
     case({AN_en,LT_en})
             'b01: begin 
                     ...
                     if(read_data == 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(lt_status1) data failed"); 
                     ...
                     if(read_data != 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(an_status1) data failed"); 
                     p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_status2"),read_data,speed,1);
                     if(read_data != 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(an_status2) data failed"); 
                   end
             'b11: begin 
   ...
   begin
     p_sequencer.top_env.reconfig_vip_for_lt_mode(speed,node);
     wait(p_sequencer.top_env.env_ip[inst].spy_if.lt_training == 1'b1);
   enable_lt_rx_checker(inst);
   enable_lt_tx_checker(inst);
    	//`ifdef G25  vinoth2x
          //reg_predict_read($sformatf("%s","lt_status1"),speed,node,'h0); //FB_FIX-577811 for C2
     wait_for_lt_vip_event(0,speed,node,inst);
   end
      ...
      if (!AN_en && !LT_en) begin
        wait(p_sequencer.top_env.env_ip[inst].spy_if.lt_trained == 0); 
    `uvm_info(get_name(), $sformatf("******* AN & LT disabled ***************"), UVM_NONE);
      end
   ...
   case({AN_en,LT_en})
       'b00 : begin 
               //reg_predict_read($sformatf("%s","an_status"),speed,node,'h0030);
               p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"lt_status1"),read_data,speed,1);
	       if(read_data != last_lt_status) `uvm_error(get_name(), "GET_REG_ANLT(lt_status1) data failed"); 
                 p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_status1"),read_data,speed,1);
               if(read_data != 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(an_status1) data failed"); 
                 p_sequencer.top_env.reg_read_anlt($sformatf("port%0d_%0s_csr",node,"an_status2"),read_data,speed,1);
               if(read_data != 'h0000_0000) `uvm_error(get_name(), "GET_REG_ANLT(an_status2) data failed"); 
     ...
     endcase
               ...
               `uvm_info("lt_mid_regster_reset_sequence", $sformatf(" Waiting for seq ready"), UVM_NONE);
  ...
  virtual task body();
     ...
     int  node_idx_200g;
     int  node_idx_400g;
     string func_name = "lt_mid_register_reset_sequence_body";
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 10G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_10g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_10g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 25G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_25g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_25g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 40G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_40g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_40g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 50G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_50g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_50g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 100G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_100g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_100g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 200G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_200g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_200g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
               ...
               fork
                 begin
                   if(p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]) begin
                     `uvm_info(get_type_name(), $sformatf("Speed 400G - lt_mid_register_reset Inst: %0d, Node_idx: %0d, active_node: %0b, anlt_node: %0b",i,idx,p_sequencer.top_env.kr_cfg_inst.active_400g[idx],p_sequencer.top_env.kr_cfg_inst.node_sel_400g[idx]), UVM_NONE);
                     lt_mid_register_reset(p_sequencer.top_env.env_ip[i].dyn_rcfg_obj_inst.speed,idx,i);
                   end
                   else begin
...
endclass:lt_mid_register_reset_sequence
