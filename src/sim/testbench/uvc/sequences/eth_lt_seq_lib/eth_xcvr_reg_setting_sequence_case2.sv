...
class eth_xcvr_reg_setting_sequence_case2 extends eth_lt_base_sequence;
  ...
  bit [3:0] lane_select;
  `uvm_object_utils(eth_xcvr_reg_setting_sequence_case2)


  function new(string name = "eth_xcvr_reg_setting_sequence_case2");
    super.new(name);
...
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_read(`REGISTERS_anlt_seq_cfg_OFFSET_REG,read_data);
   ...
   `uvm_info(get_name(), $sformatf("CO_EFF_POS                   :%0d ",k), UVM_NONE);
   send_lt_command_from_vip(CO_EFF_NEG,lane_select,DEC,HOLD,HOLD); 
...
// FIXME-MISSING_REG_IN_GDR   p_sequencer.env.reg_write(`REGISTERS_lt_cfg2_OFFSET_REG,restart_lane[3:0]);
   ...
   co_eff_predict(INITIALIZE,restart_lane,HOLD,HOLD,HOLD); 
   read_xcvr_reg();
