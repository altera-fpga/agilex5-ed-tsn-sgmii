...
class eth_rsfec_uncorrectable_codeword_sequence extends eth_base_sequence;
 uvm_reg 	regs;
 ...
 int symbol_pos[]; 
  ...
  `uvm_object_utils(eth_rsfec_uncorrectable_codeword_sequence)

  constraint num_symbol_corr_cnt_c {
    (p_sequencer.env.dyn_rcfg_obj_inst.fec_type == RSFECKP) ->  no_of_symbol_corrpt_cnt dist {16:=20, [17:20]:=80};     
    (p_sequencer.env.dyn_rcfg_obj_inst.fec_type == RSFECKR) ->  no_of_symbol_corrpt_cnt dist {8:=20, [9:15]:=80};     
    (p_sequencer.env.dyn_rcfg_obj_inst.fec_type == LLFEC)   ->  no_of_symbol_corrpt_cnt dist {8:=20, [9:15]:=80};     
  }

  function new(string name = "eth_rsfec_uncorrectable_codeword_sequence");
    super.new(name);
     ...
     send_eth_frame(RANDOM_FRAME,ETH_VIP_MAC_BOTH,20); 
      ...
      for(int i = 0; i < seq_cnt; i++) begin
         ...
         fork
          ...
          total_uncorrected_cw = total_uncorrected_cw + 20;
