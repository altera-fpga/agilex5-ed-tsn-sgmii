...
class eth_stat_mcast_ctrl_cnt_sequence extends eth_stat_base_sequence;
  ...
  `uvm_object_utils(eth_stat_mcast_ctrl_cnt_sequence)

  function new(string name = "eth_stat_mcast_ctrl_cnt_sequence");
    super.new(name);
  ...
  virtual task body();
    `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "Started eth_stat_mcast_ctrl_cnt_sequence...", UVM_NONE)
    super.body();
    ...
    `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "2. Read MCAST CTRL ERR & OK registers.", UVM_NONE)
    read_and_compare_destination_address_regs();

    `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "3. Send MCAST Control frame with an FCS error or without an FCS Error randomly.", UVM_NONE)
    repeat(itr_cnt)
    begin
       fork 
       begin
         std::randomize(eth_frame_rx) with {eth_frame_rx inside {BCAST_CTRL_FRAME,BCAST_DATA_FRAME,UCAST_CTRL_FRAME,UCAST_DATA_FRAME,MCAST_CTRL_FRAME,MCAST_DATA_FRAME};};
         randcase 
           // Send one multicast control frame with FCS error and without FCS randomly.
         ...
         endcase
         frame_num_rx++;
         `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", $sformatf("3. frame_num_rx=%0d", frame_num_rx), UVM_LOW)
       end
       begin
         std::randomize(eth_frame_tx) with {eth_frame_tx inside {BCAST_CTRL_FRAME,BCAST_DATA_FRAME,UCAST_CTRL_FRAME,UCAST_DATA_FRAME,MCAST_CTRL_FRAME,MCAST_DATA_FRAME};};
         randcase 
         ...
         endcase
         frame_num_tx++;
         `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", $sformatf("3. frame_num_tx=%0d", frame_num_tx), UVM_LOW)
       end
       join
       //Step 4:Read MCAST_CTRL_ERR and MCAST_CTRL_OK  counter registers.
       `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "4. Read MCAST CTRL ERR & OK registers.", UVM_NONE)
...
//      end
    ...
    //5. Read all stats counter registers
    `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "5. Read All stats counter registers.", UVM_NONE)
    ...
    `uvm_info("eth_stat_mcast_ctrl_cnt_sequence", "Ended eth_stat_mcast_ctrl_cnt_sequence...", UVM_NONE)
  endtask:body

endclass:eth_stat_mcast_ctrl_cnt_sequence
