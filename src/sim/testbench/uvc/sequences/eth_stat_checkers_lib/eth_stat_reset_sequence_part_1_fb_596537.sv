...
class eth_stat_reset_sequence_part_1_fb_596537 extends eth_stat_base_sequence;
  `uvm_object_utils(eth_stat_reset_sequence_part_1_fb_596537)
  ...
  virtual task body();
    ...
    `uvm_info("body", "started eth_stat_reset_sequence_part_1_fb_596537 ...", UVM_NONE)
    super.body();
  ...
  `endif
    ...
    dis_stats_chk=1;//Disabling stats read at the end of test to save sim time
    ...
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "3. Send random frames such that all stats counts have non reset value", UVM_NONE)
    send_directed_frames(.do_not_insert_fcs_err(1));
    send_random_frames(transaction_count,.do_not_insert_fcs_err(1));

    #700ns; 
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "3. send write request to these registers to make it is not overwritten by write request", UVM_NONE)
    p_sequencer.env.write_stat_regs();

    //4. Read all stats counter register .
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "4. Read all stats counter register", UVM_NONE)
    read_and_compare_stats();

    //5. Apply eio_sys_rst,csr reset or set CTRL_CONFIG register bit 0 to clear the stats counter.
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "5. Apply eio_sys_rst,csr reset or set CTRL_CONFIG register bit 0 to clear the stats counter", UVM_NONE)
    apply_rst_to_clear_stat_reg();

    //6. Repeat step 3 and 4.
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "6. Repeat step 3 and 4", UVM_NONE)
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "6.3. Send random frames such that all stats counts have non reset value", UVM_NONE)
    send_directed_frames(.do_not_insert_fcs_err(1));
    send_random_frames(transaction_count,.do_not_insert_fcs_err(1));

    #700ns; 
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "6.3. send write request to these registers to make it is not overwritten by write request", UVM_NONE)
    p_sequencer.env.write_stat_regs();

    //4. Read all stats counter register .
    `uvm_info("eth_stat_reset_sequence_part_1_fb_596537", "6.4. Read all stats counter register", UVM_NONE)
    read_and_compare_stats();
    #400ns;

    `uvm_info("body", "ended eth_stat_reset_sequence_part_1_fb_596537 ...", UVM_NONE)
  endtask

endclass:eth_stat_reset_sequence_part_1_fb_596537
