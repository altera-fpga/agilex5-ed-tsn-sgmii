...
class eth_stat_bcast_ctrl_cnt_sequence extends eth_stat_base_sequence;
   frame_type eth_frame;
   int frame_num_rx=0;
   int frame_num_tx=0;
   int itr_cnt;

  `uvm_object_utils(eth_stat_bcast_ctrl_cnt_sequence)

  function new(string name = "eth_stat_bcast_ctrl_cnt_sequence");
    super.new(name);
  ...
  virtual task body();
    `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", "Started eth_stat_bcast_ctrl_cnt_sequence...", UVM_NONE)
    super.body();
    ...
    //Step 3:Send one broad cast control frame with FCS error and without FCS randomly.
    `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", "3. Send BCAST Control frame with an FCS error or without an FCS Error randomly.", UVM_NONE)
    repeat(itr_cnt)
    begin
       ...
       begin
         ...
         endcase
         frame_num_rx++;
         `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", $sformatf("3. frame_num_rx=%0d", frame_num_rx), UVM_LOW)
       end
       begin
         randcase
         ...
         endcase
         frame_num_tx++;
         `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", $sformatf("3. frame_num_tx=%0d", frame_num_tx), UVM_LOW)
       end
       join
       //Step 4:Read BCAST_CTRL_ERR and BCAST_CTRL_OK  counter registers.
       `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", "4. Read BCAST CTRL ERR & OK registers.", UVM_NONE)
   ...
   //   end
    ...
    `uvm_info("eth_stat_bcast_ctrl_cnt_sequence", "Ended eth_stat_bcast_ctrl_cnt_sequence...", UVM_NONE)
  endtask:body

endclass:eth_stat_bcast_ctrl_cnt_sequence
