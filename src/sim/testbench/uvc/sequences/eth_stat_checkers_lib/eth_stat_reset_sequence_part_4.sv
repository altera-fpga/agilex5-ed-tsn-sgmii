...
class eth_stat_reset_sequence_part_4 extends eth_stat_base_sequence;
  `uvm_object_utils(eth_stat_reset_sequence_part_4)
  ...
  virtual task body();
    ...
    `uvm_info("body", "started eth_stat_reset_sequence_part_4 ...", UVM_NONE)
    super.body();
    ...
    //  `uvm_info("eth_stat_reset_sequence_part_4", "2. read and compare all stats at the begining of sequence", UVM_NONE)
    ...
    `uvm_info("eth_stat_reset_sequence_part_4", "3. Send random frames such that all stats counts have non reset value", UVM_NONE)
    send_directed_frames();
    send_random_frames(transaction_count);

    #700ns; 
    `uvm_info("eth_stat_reset_sequence_part_4", "3. send write request to these registers to make it is not overwritten by write request", UVM_NONE)
    p_sequencer.env.write_stat_regs();

    //4. Read all stats counter register .
    `uvm_info("eth_stat_reset_sequence_part_4", "4. Read all stats counter register", UVM_NONE)
    read_and_compare_stats();


    //14. repeat step 7 to 10.
    `uvm_info("eth_stat_reset_sequence_part_4", "14. repeat step 7 to 10", UVM_NONE)

    `uvm_info("eth_stat_reset_sequence_part_4", "14.1. Apply shadow request again and apply reset/clear stat regs while shadow request is ON", UVM_NONE)
    apply_shadow_request();

    //7. Apply hard tx ,hard rx, soft tx or soft rx mac reset randomly
    `uvm_info("eth_stat_reset_sequence_part_4", "14.7. Apply hard tx ,hard rx, soft tx or soft rx mac reset randomly", UVM_NONE)
    ...
    `uvm_info("eth_stat_reset_sequence_part_4", "14.8. Read all stats counter register to check they retain value", UVM_NONE)
    read_and_compare_stats();

    `uvm_info("eth_stat_reset_sequence_part_4", "14.9. Apply eio_sys_rst, csr reset or set CTRL_CONFIG register bit 0 and check stats counters are not reset and 14.10. Read all stats counter registers", UVM_NONE)
    apply_rst_to_clear_stat_reg();

    `uvm_info("eth_stat_reset_sequence_part_4", "14.10. clear shadow request", UVM_NONE)
    clear_shadow_request();

    #0;
    `uvm_info("eth_stat_reset_sequence_part_4", "14.11. Read all stats counter registers, they should have actual counter values as shadow request is cleared", UVM_NONE)
    read_and_compare_stats();

    `uvm_info("eth_stat_reset_sequence_part_4", "15.1. Check clear stats functionality. Send frames to get counters incremented", UVM_NONE)
    send_directed_frames();
    send_random_frames(transaction_count);

    `uvm_info("eth_stat_reset_sequence_part_4", "15.2. Clear stats registers", UVM_NONE)
    clear_stat_counters();
    #400ns;

    `uvm_info("body", "ended eth_stat_reset_sequence_part_4 ...", UVM_NONE)
  endtask
...
endclass:eth_stat_reset_sequence_part_4
