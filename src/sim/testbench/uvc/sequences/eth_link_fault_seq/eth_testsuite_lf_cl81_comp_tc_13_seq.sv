...
class eth_testsuite_lf_cl81_comp_tc_13_seq extends eth_base_sequence;
  
  `uvm_object_utils(eth_testsuite_lf_cl81_comp_tc_13_seq)

  integer first_case, last_case;
  
  function new(string name = "eth_testsuite_lf_cl81_comp_tc_13_seq");
    super.new(name);
	`ifdef UVM_POST_VERSION_1_1
     set_automatic_phase_objection(1);
    `endif
    if($value$plusargs("ETH_FIRST_TEST=%d",first_case)) begin
      `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", $psprintf("First case selected from run define %d",first_case), UVM_NONE)
    end
    else begin
      first_case = 13;
    end
    if($value$plusargs("ETH_LAST_TEST=%d",last_case)) begin
      `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", $psprintf("Last case selected from run define %d",last_case), UVM_NONE)
    end
    else begin
      last_case = 13;
    end
  ...
  virtual task body();
    ...
    `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "Executing eth_testsuite_lf_cl81_comp_tc_13_seq ...", UVM_NONE)
//    apply_hard_reset(0,0,1,11);
   ...
   //p_sequencer.env.m_snps_eth_pcs66_agent.eth_agent[0].monitor.err_check_rx.svt_err_xgmii_start_c_char_after_start_c_char.set_default_fail_effect(svt_err_check_stats::IGNORE);
	...
	//muralasx: FIXME start_testsuite_test(), testsuite_case_select(), monitor_error_event()& wait_for_testsuite_test_finish() methods in testsuitetasks interface and release below lines. 
        ...
        `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "start_testsuite_test done ...", UVM_NONE)
   ...
   //   begin
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "waiting for local fault is deasserted ...", UVM_NONE)
   //       wait(p_sequencer.env.spy_if.local_fault == 1'b1)
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "local fault is asserted ...", UVM_NONE)
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "local fault disabling scoreboard ...", UVM_NONE)
   //       p_sequencer.env.sb_mac_tx_vip_rx.scb_en = 0;
   //       uvm_config_db#(int)::set(null, " *", "scb_en", 0);
   //       wait(p_sequencer.env.spy_if.local_fault == 1'b0)
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "local fault is deasserted ...", UVM_NONE)
   //       wait(p_sequencer.env.spy_if.avst_tx_eop == 1'b1);
   //       wait(p_sequencer.env.spy_if.avst_tx_sop == 1'b1);
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "local fault enabling scoreboard ...", UVM_NONE)
   //       uvm_config_db#(int)::set(null, "*", "scb_en", 1);
   //       p_sequencer.env.sb_mac_tx_vip_rx.scb_en = 1;
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "waiting for remote fault ...", UVM_NONE)
   //       wait(p_sequencer.env.spy_if.remote_fault == 1'b1)
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "remote fault disabling scoreboard ...", UVM_NONE)
   //       p_sequencer.env.sb_mac_tx_vip_rx.scb_en = 0;
   //       uvm_config_db#(int)::set(null, " *", "scb_en", 0);
   //       wait(p_sequencer.env.spy_if.remote_fault == 1'b0)
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", " waiting for remote fault is deassertion ...", UVM_NONE)
   //       wait(p_sequencer.env.spy_if.avst_tx_eop == 1'b1);
   //       wait(p_sequencer.env.spy_if.avst_tx_sop == 1'b1);
   //       `uvm_info("eth_testsuite_lf_cl81_comp_tc_13_seq", "remote fault enabling scoreboard ...", UVM_NONE)
   //       uvm_config_db#(int)::set(null, "*", "scb_en", 1);
   //       p_sequencer.env.sb_mac_tx_vip_rx.scb_en = 1;
   //   end
    ...
    #1us;
  endtask
endclass : eth_testsuite_lf_cl81_comp_tc_13_seq
