...
class ptp_tx_flow_control_sequence extends eth_ptp_base_sequence;
  `uvm_object_utils(ptp_tx_flow_control_sequence)
...
function new(string name = "seq_0");
  ...
  // rx_fc_en=$urandom;//dynamic xoff/xon generation 
   ...
   reg_mode=$urandom_range('h7,'h1); //for tb use only
  ...
  `uvm_info("eth_seq_lib", "running fc_rand sequence\n",UVM_LOW)
   ...
   //if(rd_data[0]!=0)   `uvm_error("ptp_tx_flow_control_sequence", $sformatf("REGISTERS_tx_xof_en_tx_pause_qnumber_OFFSET_REG bit 0 value must be initialized as per parameter"));   // As per FB 586418 ,this register will always have value 0.  this is not required as this register will be controlrolled by GDR_RUN
 ...
 //  if(rd_data[0]!=p_sequencer.env.dyn_rcfg_obj_inst.rx_fc_fwd)   `uvm_error("fc_rand_sequence", $sformatf("REGISTERS_rx_pause_fwd_OFFSET_REG value must be initialized as per parameter"));
   ...
   `uvm_info("ptp_tx_flow_control_sequence", "=========================FLOW CONTROL INFO FOR THIS SIMULATION===================\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence", "=========================TX PATH===================\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("Ports Enabled for input:%h...\n",port_en),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("Ports Enabled for sending automatic holdoff xoff frames:%h...\n",holdoff_en),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("SFC pause quanta:%h...\n",sfc_pause_quanta),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("SFC hold quanta:%h...\n",sfc_holdoff_quanta),UVM_LOW)
   if(ready_drop) `uvm_info("ptp_tx_flow_control_sequence","SFC will inhibit traffic flow...\n",UVM_LOW)
   else `uvm_info("ptp_tx_flow_control_sequence","SFC will not stop TX traffic...\n",UVM_LOW)
   if(same_holdoff_en) `uvm_info("ptp_tx_flow_control_sequence",$sformatf("all PFC queues will have same holdoff value which is:%h",same_holdoff_quanta),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("TX SOURCE ADDR:%h...\n",tx_sa),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("TX DEST ADDR:%h...\n",tx_da),UVM_LOW)
   if(tx_fc_en[1]) `uvm_info("ptp_tx_flow_control_sequence","PFC TX frames will be passed by DUT...\n",UVM_LOW)
   if(tx_fc_en[0]) `uvm_info("ptp_tx_flow_control_sequence","SFC TX frames will be passed by DUT...\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence", "=========================RX PATH===================\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("Ports Enabled for output:%h...\n",rx_pfc_en),UVM_LOW)
   if(rx_fc_fwd) `uvm_info("ptp_tx_flow_control_sequence","Control frames will be forwarded on rx user interface...\n",UVM_LOW)
   else `uvm_info("ptp_tx_flow_control_sequence","Control frames will not be forwarded on rx user interface...\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("RX DEST ADDR in VIP mode:%h...\n",rx_da),UVM_LOW)
   if(rx_fc_en[1]) `uvm_info("ptp_tx_flow_control_sequence","PFC RX frames will be passed by DUT...\n",UVM_LOW)
   if(rx_fc_en[0]) `uvm_info("ptp_tx_flow_control_sequence","SFC RX frames will be passed by DUT...\n",UVM_LOW)
   ...
   `uvm_info("ptp_tx_flow_control_sequence", "All fc registers are written, triggered event..\n",UVM_LOW)

   `uvm_info("ptp_tx_flow_control_sequence", "This test will run in Pause Mode...\n",UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("PAUSE TIME=%dns\n",12.8*sfc_pause_quanta),UVM_LOW)
   `uvm_info("ptp_tx_flow_control_sequence",$sformatf("HOLD TIME=%dns\n",12.8*sfc_holdoff_quanta),UVM_LOW)
   p_sequencer.env.m_ptp_tx_ref_model.disable_vl_check = 1;
   `ifdef ENABLE_ETH_VIP
	 ...
	 `endif
   send_ptp_frame(INS_V2,DATA_FRAME,10);
     fork
		         pause_seq.start(p_sequencer.fc_sqr);
             send_ptp_frame(INS_V2,DATA_FRAME,500);  
	   join

     p_sequencer.env.sb_loopbk.pause_q_tx.delete(); //SCB error- rx fwd is 0 so tx packets > rx packets 
	 endtask
endclass
