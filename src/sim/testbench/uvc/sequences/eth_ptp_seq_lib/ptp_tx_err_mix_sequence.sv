...
class ptp_tx_err_mix_sequence extends eth_ptp_base_sequence;
  ptp_op_e ptp_op;
  bit mix_rule;
  int num_itr;
  bit eop_detect;

  `uvm_object_utils(ptp_tx_err_mix_sequence)
  function new(string name = "ptp_tx_err_mix_sequence");
    super.new(name);
 ...
 virtual task body();
   ...
   `endif
  `uvm_info("eth_seq_lib", "running ptp_tx_err_mix_sequence\n",UVM_LOW)
   mix_rule = 0;
  ...
  begin
    repeat(50) begin
      ...
      //randcase
        send_eth_frame_with_tx_error(1,RANDOM_FRAME,AVL_TX_ETH_VIP,,1,ptp_op);  // PTP Error frame with tx error
       ++num_itr;	  
    end
  end
  begin
...
`endif
 p_sequencer.env.m_ptp_tx_ref_model.bypass_queue = 0;
  ...
  endtask:post_start
    `endif
endclass : ptp_tx_err_mix_sequence
