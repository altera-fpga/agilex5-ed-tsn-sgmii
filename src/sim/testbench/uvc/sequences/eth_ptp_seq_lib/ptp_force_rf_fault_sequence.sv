...
class ptp_force_rf_fault_sequence extends eth_ptp_base_sequence;
  ptp_op_e ptp_op;
  frame_type f_type;
  bit fault;
  bit mix_rule;
  ...
  `uvm_object_utils(ptp_force_rf_fault_sequence)
  function new(string name = "ptp_force_rf_fault_sequence");
    super.new(name);
...
//p_sequencer.env.sb_vip_tx_mac_rx.scb_dis=1;
   ...
   `uvm_info("eth_seq_lib", "running ptp_force_rf_fault_sequence\n",UVM_LOW)
   //mix_rule = $urandom;
...
begin
  ...
  begin
    ...
    p_sequencer.env.reg_read(`ETH_F_ALL_link_fault_status_OFFSET_REG,read_data);
      ...
      `uvm_info("ptp_force_rf_fault_sequence", "Done 4\n",UVM_LOW)
      
    #5us;
    
   endtask
endclass : ptp_force_rf_fault_sequence
