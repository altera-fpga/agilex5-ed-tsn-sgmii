...
class ptp_cf_ts_rollover_sequence extends eth_ptp_base_sequence;
  frame_type f_type;
  ptp_op_e ptp_op;
  int fb609905_rule;

  `uvm_object_utils(ptp_cf_ts_rollover_sequence)
  function new(string name = "ptp_cf_ts_rollover_sequence");
    super.new(name);
  ...
  virtual task body();
    super.body();
    `uvm_info("eth_seq_lib", "running ptp_cf_ts_rollover_sequence\n",UVM_LOW)
 ...
 if (p_sequencer.env.dyn_rcfg_obj_inst.mode == PCSMAC) begin
     p_sequencer.env.m_ptp_tx_agent.m_ptp_tod_drv.m_ptp_config.tod_ns_part= 'd999935695 - $floor($realtime/1000); 
	   p_sequencer.env.m_ptp_tx_agent.m_ptp_tod_drv.m_ptp_config.ing_ns_part= 'd999919865 - $floor($realtime/1000);
 end else if(p_sequencer.env.dyn_rcfg_obj_inst.mode ==MACSEG) begin
	   p_sequencer.env.m_ptp_tx_agent.m_ptp_tod_drv.m_ptp_config.tod_ns_part= 'd999970913 - $floor($realtime/1000);   
     p_sequencer.env.m_ptp_tx_agent.m_ptp_tod_drv.m_ptp_config.ing_ns_part=   'd999963183 - $floor($realtime/1000);   
end
...
endclass : ptp_cf_ts_rollover_sequence
