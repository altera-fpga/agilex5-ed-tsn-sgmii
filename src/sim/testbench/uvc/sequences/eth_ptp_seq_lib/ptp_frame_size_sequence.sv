...
class ptp_frame_size_sequence extends eth_ptp_base_sequence;
  `uvm_object_utils(ptp_frame_size_sequence)
...
//        end
        ...
        `uvm_info("ptp_frame_size_sequence", "Waiting for RX PTP Ready\n",UVM_LOW)
        wait (p_sequencer.env.spy_if.o_rx_ptp_ready === 1'b1);
        `uvm_info("ptp_frame_size_sequence", "RX PTP ready\n",UVM_LOW)
 ...
 //std::randomize(f_type) with {f_type inside {ETH_VLAN_FRAME,ETH_STACKED_VLAN_FRAME,ETH_DATA_FRAME,ETH_JUMBO_DATA_FRAME,ETH_JUMBO_VLAN_FRAME,ETH_JUMBO_STACKED_VLAN_FRAME,ETH_SFC_FRAME,ETH_PFC_FRAME,ETH_IPV4_FRAME,ETH_IPV6_FRAME,ETH_USER_DEFINED_FRAME};}; // RAMI - No IPV4/IPV6 PTP frames as we see VIP errors (from TX side we treat them same as DATA frame)
std::randomize(f_type) with {f_type inside {ETH_VLAN_FRAME,ETH_STACKED_VLAN_FRAME,ETH_DATA_FRAME,ETH_JUMBO_DATA_FRAME,ETH_JUMBO_VLAN_FRAME,ETH_JUMBO_STACKED_VLAN_FRAME};};
 //std::randomize(mix_rule) with {mix_rule inside {1,2,3};}; 
 std::randomize(mix_rule) with {mix_rule ==2 ;}; //1 is all v1 frames, 3 without v1 is 2
     
   if((f_type == ETH_JUMBO_DATA_FRAME) || (f_type == ETH_JUMBO_VLAN_FRAME) || (f_type == ETH_JUMBO_STACKED_VLAN_FRAME))
     size =1501;
   else if (f_type == ETH_VLAN_FRAME)
     size =42;
...
endtask
endclass : ptp_frame_size_sequence
