...
class ptp_reset_2step_accuracy_sequence extends eth_ptp_base_sequence;
  ...
  `uvm_object_utils(ptp_reset_2step_accuracy_sequence)

  eth_ptp_config_sequence eth_ptp_config_seq;
  
  function new(string name = "ptp_reset_2step_accuracy_sequence");
    super.new(name);
  ...
  virtual task body();
   ...
   `uvm_info(get_full_name(), "Executing ptp_reset_2step_accuracy_sequence ...", UVM_LOW)
...
//    `uvm_info("serial_wait", "Done waiting additional 20 TX_AMs before TX traffic\n",UVM_NONE)
   ...
   `uvm_info(get_full_name(), "Exiting ptp_reset_2step_accuracy_sequence ...", UVM_LOW)
  endtask
endclass
