...
class ptp_tx_fifo_stress_sequence extends eth_ptp_base_sequence;
  `uvm_object_utils(ptp_tx_fifo_stress_sequence)
   ...
   bit eop_detect;
   int cnt =0;
  function new(string name = "ptp_tx_fifo_stress_sequence");
    super.new(name);
 ...
 virtual task body();
   `uvm_info("body", "started ptp_tx_fifo_stress_sequence ...", UVM_MEDIUM)
...
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("Reading rsfec_ln_mapping_rx_%0d", ln),UVM_MEDIUM)
...
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("rsfec_ln_mapping_rx_%0d = 32'h%0h",ln,read_data),UVM_MEDIUM)
//            if(read_data == 32'h0) phy_ln0_map = ln;
//
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("Reading rsfec_ln_skew_rx_%0d", ln),UVM_MEDIUM)
...
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("rsfec_ln_skew_rx_%0d = 32'h%0h",ln,skew_ln[ln]),UVM_MEDIUM)
//
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("Reading rsfec_cw_pos_rx_%0d", ln),UVM_MEDIUM)
//            cw_pos_rx[ln] = 32'h0;
...
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("rsfec_cw_pos_rx_%0d = 32'h%0h",ln,cw_pos_rx[ln]),UVM_MEDIUM)
//        end
//
//        min_val = skew_ln.min(); 
//        min_skew = min_val.pop_front();
//        `uvm_info("eth_ptp_base_vl_offset", $sformatf("min skew value = 32'h%0h",min_skew),UVM_MEDIUM)
//
//        lane_skew_adjust = skew_ln[phy_ln0_map] - min_skew + (skew_ln.sum()/4);
//        `uvm_info("eth_ptp_base_vl_offset", $sformatf("lane_skew_adjust = 32'h%0h",lane_skew_adjust),UVM_MEDIUM)
//
...
//        `uvm_info("eth_ptp_base_vl_offset", $sformatf("Tlat_final = 32'h%0h",Tlat_final),UVM_MEDIUM)
//
...
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("Reading VL offset data for VL %0d",vl),UVM_MEDIUM)
...
//
//            `uvm_info("eth_ptp_base_vl_offset", $sformatf("Calculating VL offset data for VL %0d",vl),UVM_MEDIUM)
...
//`endif
      ...
      p_sequencer.env.spy_if.ptp_acc_check_disable = 1;

      //Disable SVA checker
      p_sequencer.env.spy_if.dis_sva = 1;

        `uvm_info("ptp_tx_fifo_stress_sequence", "Waiting for RX PTP Ready\n",UVM_LOW)
        wait (p_sequencer.env.spy_if.o_rx_ptp_ready === 1'b1);
        `uvm_info("ptp_tx_fifo_stress_sequence", "RX PTP ready\n",UVM_LOW)
  ...
  eth_seq.ipg = 0;
   ...
   begin
      if (p_sequencer.env.dyn_rcfg_obj_inst.mode == PCSMAC) begin 
    	...
    	wait(eop_detect == 1)
      `uvm_info("ptp_tx_fifo_stress_sequence", "eop detect\n",UVM_LOW)
   end
   begin
      ...
      if(p_sequencer.env.dyn_rcfg_obj_inst.mode ==MACSEG)begin      
         repeat(300)begin
            ...
            cnt ++;
            `uvm_info(get_type_name(),$sformatf("seg packet cnt =  %d",cnt), UVM_LOW) 
         end         
      end else begin
...
#10ns;
      ...
      p_sequencer.env.spy_if.ptp_acc_check_disable = 0; 
    //Enable SVA
         #1us;
    p_sequencer.env.spy_if.dis_sva = 0;    

#5ns;
p_sequencer.env.m_ptp_tx_ref_model.reset_model_i= 0;
  
`uvm_info("ptp_tx_fifo_stress_sequence", "Initiate good frames to transmit\n",UVM_LOW)

 ///normal packet GOOD Frames ///////
 if (p_sequencer.env.dyn_rcfg_obj_inst.mode == PCSMAC) begin 
     ...
     eth_normal_seq.ipg = 0;
     if(mix_rule == 3)
       eth_normal_seq.ptp = 1'b1;
     eth_normal_seq.fr_type = f_type;
     eth_normal_seq.sequence_length = 100;
...
endclass : ptp_tx_fifo_stress_sequence
