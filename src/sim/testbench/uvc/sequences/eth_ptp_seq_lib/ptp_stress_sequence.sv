...
class ptp_stress_sequence extends eth_ptp_base_sequence;
  `uvm_object_utils(ptp_stress_sequence)
...
int mix_rule;
  ...
  function new(string name = "ptp_stress_sequence");
    super.new(name);
 ...
 virtual task body();
   `uvm_info("body", "started ptp_stress_sequence ...", UVM_NONE)
...
//        end
        ...
        `uvm_info("ptp_stress_sequence", "Waiting for RX PTP Ready\n",UVM_LOW)
        wait (p_sequencer.env.spy_if.o_rx_ptp_ready === 1'b1);
        `uvm_info("ptp_stress_sequence", "RX PTP ready\n",UVM_LOW)
     ...
     for(int i=0; i < 10 ; i++) begin
       ...
       std::randomize(f_type) with {f_type inside {JUMBO_DATA_FRAME,JUMBO_VLAN_FRAME,JUMBO_STACKED_VLAN_FRAME};};
       std::randomize(size) with {size inside {[8000:9582]};};
