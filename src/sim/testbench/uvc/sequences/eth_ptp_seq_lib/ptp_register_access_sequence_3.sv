...
class ptp_register_access_sequence_3 extends eth_base_sequence;
  ...
  `uvm_object_utils(ptp_register_access_sequence_3)

   function new(string name = "ptp_register_access_sequence_3");
      super.new(name);
...
//      `uvm_info("ptp_register_access_sequence_3", $sformatf("Register compare_disable[('h%h)-(%s)] = %0d",regs[i].get_address(),regs[i].get_name(),compare_disable[regs[i].get_address()]), UVM_NONE)
//    end
//    
...
//    end
      ...
      begin
         ...
         `GET_REG_ADDR(mac_cfg_rx_pkt_n_ts_rx_ctr_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed)}))begin
            compare_disable[regs[i].get_address()]=1;
            `uvm_info("ptp_register_access_sequence_3", $sformatf("Disabling Register %s compare_disable",regs[i].get_name()), UVM_NONE)
         end
      ...
      //Performing walk 0 pattern.
      randomize(k) with {k inside {0,4,8,12,16,20,24,28};};
      `uvm_info("ptp_register_access_sequence_3", "Performing walk 0 Pattern...", UVM_LOW)
      foreach(regs[i])
      begin
         for(int j=k;j<=k+3;j++)
         begin
