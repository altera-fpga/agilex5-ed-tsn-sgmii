...
class ptp_dut_recv_neg_ppm_sequence extends eth_ptp_base_sequence;

  `uvm_object_utils(ptp_dut_recv_neg_ppm_sequence)
  function new(string name = "ptp_dut_recv_neg_ppm_sequence");
    super.new(name);
...
p_sequencer.env.reg_write(`GET_REG_ADDR(mac_cfg_rx_ptp_ui_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),'h9EE41B);
end
else if (p_sequencer.env.spy_if.speed == _50G) begin
`uvm_info("eth_seq_lib", "changing now the value of ptp_ui reg for 50G\n",UVM_LOW)
p_sequencer.env.reg_write(`GET_REG_ADDR(mac_cfg_rx_ptp_ui_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed),'h4F720E);

end

   super.body();
    dis_stats_chk=1;
   `uvm_info("eth_seq_lib", "running now ptp_dut_recv_neg_ppm_sequence\n",UVM_LOW)
...
//     `uvm_info("serial_wait", "Done waiting additional 20 RX_AMs before RX traffic\n",UVM_NONE)
  ...
  endtask
endclass : ptp_dut_recv_neg_ppm_sequence
