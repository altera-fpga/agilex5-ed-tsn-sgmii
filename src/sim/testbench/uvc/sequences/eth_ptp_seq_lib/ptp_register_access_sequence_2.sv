...
class ptp_register_access_sequence_2 extends eth_base_sequence;
  ...
  `uvm_object_utils(ptp_register_access_sequence_2)

   function new(string name = "ptp_register_access_sequence_2");
      super.new(name);
	...
	//	compare_disable[regs[i].get_address()]=1;
      ...
      begin
         ...
         `GET_REG_ADDR(mac_cfg_rx_pkt_n_ts_rx_ctr_OFFSET_REG,p_sequencer.env.dyn_rcfg_obj_inst.speed)}))begin
            compare_disable[regs[i].get_address()]=1;
            `uvm_info("ptp_register_access_sequence_2", $sformatf("Disabling Register %s compare_disable",regs[i].get_name()), UVM_NONE)
         end
      ...
      //Performing walk 1 pattern.
      randomize(k) with {k inside {0,4,8,12,16,20,24,28};};
      `uvm_info("ptp_register_access_sequence_2", "Performing walk 1 Pattern...", UVM_LOW)
      foreach(regs[i]) 
      begin
         for(int j=k;j<=k+3;j++) 
         begin
