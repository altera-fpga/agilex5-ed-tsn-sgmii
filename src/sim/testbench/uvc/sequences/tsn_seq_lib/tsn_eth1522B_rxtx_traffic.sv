class tsn_eth1522B_rxtx_traffic extends eth_base_sequence;

    `uvm_object_utils(tsn_eth1522B_rxtx_traffic)
    ...
    virtual task body();
      ...
      `ifdef ENABLE_ETH_VIP
          fork
            send_tx_vlan_pkts();
	    send_eth_frame_with_fix_size(.eth_frame(VLAN_FRAME),.frame_size(1522),.no_of_frame(num_of_frames),.path(ETH_VIP_AVL_RX));
          join
    ...
    task send_tx_vlan_pkts();
       for(int i=0; i<num_of_frames; i++) begin
	  send_eth_frame_with_fix_size(.eth_frame(VLAN_FRAME),.frame_size(1522),.no_of_frame(1),.path(AVL_TX_ETH_VIP));
	  #20ns;
       end
    endtask

  endclass: tsn_eth1522B_rxtx_traffic
