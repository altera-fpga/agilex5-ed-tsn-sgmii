...
class vip_decoder_sequence1 extends vip_error_base_sequence;
   `uvm_object_utils(vip_decoder_sequence1)
     alt_eth_error_vip_base_sequence err_seq;
   
   function new(string name = "vip_decoder_sequence1");
      super.new(name);
  ...
  `endif
   ...
   virtual task body();
      ...
      `uvm_create_on(err_seq,p_sequencer.eth_vip_seqr_inst)
       //25G minimum IFG > minimum value;VIP - SNPS ticket 01105940
      if(!(p_sequencer.env.dyn_rcfg_obj_inst.speed inside {_10G,_25G})) begin
        err_seq.send_error_frame(.exception_list(exception_list),.no_of_frames(sequence_length),.frame_size_min(frame_size_min),.frame_size_max(frame_size_max),.one_exception(1'b1),.ifg(1),.en_short_packet(en_short_packet));
      end else begin
        err_seq.send_error_frame(.exception_list(exception_list),.no_of_frames(sequence_length),.frame_size_min(frame_size_min),.frame_size_max(frame_size_max),.one_exception(1'b1),.ifg(12),.en_short_packet(en_short_packet));
      end
        #400ns;
      ...
      exception_list.add_exception(exception);
      `uvm_create_on(err_seq,p_sequencer.eth_vip_seqr_inst)      
      err_seq.send_error_frame(.exception_list(exception_list),.no_of_frames(sequence_length),.frame_size_min(frame_size_min),.frame_size_max(frame_size_max),.one_exception(1'b1),.ifg(10),.en_short_packet(en_short_packet));
      #400ns;
//      p_sequencer.env.wait_vip_tx_frames_done(.exp_num(total_num_frames_sent),.timeout_time(frame_timeout_time)); 
      p_sequencer.env.wait_client_rx_frames_done(.exp_num(p_sequencer.env.eth_ref_model_inst.vip_tx_count-p_sequencer.env.eth_ref_model_inst.drop_count),.timeout_time(frame_timeout_time));
      ////////////////////////////////////////////////////
      ////// Phase 1: B2B frames - IFG=16. ////////////////
      ////////////////////////////////////////////////////
      total_num_frames_sent=total_num_frames_sent+sequence_length;
      `uvm_create_on(err_seq,p_sequencer.eth_vip_seqr_inst)      
      err_seq.send_error_frame(.exception_list(exception_list),.no_of_frames(sequence_length),.frame_size_min(frame_size_min),.frame_size_max(frame_size_max),.one_exception(1'b1),.ifg(10),.en_short_packet(en_short_packet));
...
endclass // vip_decoder_sequence1
