...
class vip_strict_sfd_sequence extends vip_error_base_sequence;
      
   `uvm_object_utils(vip_strict_sfd_sequence)
     alt_eth_error_vip_base_sequence err_seq;
   
   function new(string name = "vip_strict_sfd_sequence");
      super.new(name);
  `ifdef UVM_POST_VERSION_1_1
      set_automatic_phase_objection(1);
  `endif
      //muralasx: Newly added code
   ...
   virtual task body();
      ...
      preamble_post = $urandom_range(55,0);
      sfd_post = $urandom_range(7,0);
      bad_preamble = 56'hAAAAAAAAAAAAAA; // It is for coverage   
      //bad_preamble[preamble_post] =~good_preamble[preamble_post];
      bad_sfd = good_sfd;
      bad_sfd[sfd_post] = ~good_sfd[sfd_post];
      exception = new();
...
endclass // vip_strict_sfd_sequence
