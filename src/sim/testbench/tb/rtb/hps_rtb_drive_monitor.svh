...
class hps_comm_concrete extends hps_comm_abstract;
   ...
   //
   function new(string name = "hps_comm_concrete");
      super.new(name);
      m_class_type = name;
   endfunction : new
   ...
   //
   /*
   virtual function void set_config(hps_env_config _config);
   ...
   */
   //
   // Macros: hps_monitor_signal
   // Handy macro that will be used to monitor signals
   //
   `define hps_monitor_signal(NAME, VAL, IS_OUT, OUT_VAL)\
      `"NAME`" : begin\
         string msg, msg_new;\
         int i, j;\
         if(IS_OUT == 1)  begin \
   ...
   `define hps_monitor_transition(NAME, B_VAL, A_VAL)\
      `"NAME`" : begin\
         string msg;\
   ...
   // Wait for cvp clock
   `define hps_wait_clock(NAME,VAL)\
      `"NAME`" : begin\
         repeat(value) @(posedge ``NAME``);\
      end

   //
   // Macros: hps_drive_signal
   // Handy macro that will be used to control reset signals
   //
   `define hps_drive_signal(NAME, VAL, CLOCK="")\
      `"NAME`" : begin\
         /**/\
      ...
      task monitor_transition(string name , bit [127:0]  b_value , bit[127:0] a_value);
        ...
        endcase
      endtask
endclass : hps_comm_concrete

// Create and get the communication interface concrete class
function hps_comm_concrete get_comm();
   // Concrete class handle
   static   hps_comm_concrete  m_comm;
...
function bit init_comm();
   ...
   `altuvm_set_concrete(hps_comm_concrete, get_comm())
   return 1;
