...
class tsn_comm_concrete extends tsn_comm_abstract;
   ...
   //
   function new(string name = "tsn_comm_concrete");
      super.new(name);
      m_class_type = name;
   endfunction : new
   ...
   //
   /*
   virtual function void set_config(tsn_env_config _config);
   ...
   */
   //
   // Macros: tsn_monitor_signal
   // Handy macro that will be used to monitor signals
   //
   `define tsn_monitor_signal(NAME, VAL, IS_OUT, OUT_VAL)\
      `"NAME`" : begin\
         string msg, msg_new;\
         int i, j;\
         if(IS_OUT == 1)  begin \
   ...
   `define tsn_monitor_transition(NAME, B_VAL, A_VAL)\
      `"NAME`" : begin\
         string msg;\
   ...
   // Wait for cvp clock
   `define tsn_wait_clock(NAME,VAL)\
      `"NAME`" : begin\
         repeat(value) @(posedge ``NAME``);\
      end

   //
   // Macros: tsn_drive_signal
   // Handy macro that will be used to control reset signals
   //
   `define tsn_drive_signal(NAME, VAL, CLOCK="")\
      `"NAME`" : begin\
         /**/\
      ...
      task monitor_transition(string name , bit [127:0]  b_value , bit[127:0] a_value);
        ...
        endcase
      endtask
endclass : tsn_comm_concrete

// Create and get the communication interface concrete class
function tsn_comm_concrete get_comm();
   // Concrete class handle
   static   tsn_comm_concrete  m_comm;
...
function bit init_comm();
   ...
   `altuvm_set_concrete(tsn_comm_concrete, get_comm())
   return 1;
