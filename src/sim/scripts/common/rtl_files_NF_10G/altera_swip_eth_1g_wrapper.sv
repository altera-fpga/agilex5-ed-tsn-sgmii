...
module altera_swip_eth_1g10g_mac_phy_de_wrapper (
   ...
   output   rx_syncstatus 
    ...
    logic clk_1g_ref;
    logic clk_1g_ref_local;
    logic clk_10g_ref;
    logic reconfig_req;
    logic reconfig_busy;
    `define ETH_XSBI_CLOCK         (1551.52/2.0) 
    //`define ETH_XSBI_CLOCK         (3103.04/2.0) 
      
    initial clk_1g_ref_local = 1'b0;
    always #(4000) clk_1g_ref_local  = ~clk_1g_ref_local ;

    `ifdef ENABLE_ETH_VIP
       assign clk_1g_ref = eth_env_top.svt_ethernet_txrx_if[0].gmii_tx_clk;
    `else
          assign clk_1g_ref = clk_1g_ref_local;
   `endif
...
`endif
 ...
 altera_eth_10g_mac_1g_nf_phy U_DUT (
