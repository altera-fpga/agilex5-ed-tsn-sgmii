...
// dsamantx :This was previously commented in earlier version of C3.
// Need to check again whether AVMM assertion cover this thing or not.if
// covered,then remove below codes
// FIXME :-Need to check after RTL is ready for use
...
endproperty
//muralasx:
//Masking due to the HSD: 
/*
readdata_x_ip0:assert property (@(posedge clk_status_ip0)read_data_x_ip0(reset_if_ip0.csr_rst_n & dut.o_rx_pcs_ready_ip0 & dut.o_tx_lanes_stable_ip0))
else
...
endproperty
/*
eopvalid_align_ip0:assert property (@(posedge `CLK_RXMAC_IP0)eop_valid_align_ip0(reset_if_ip0.csr_rst_n & dut.o_rx_pcs_ready_ip0 & dut.o_tx_lanes_stable_ip0))
else
