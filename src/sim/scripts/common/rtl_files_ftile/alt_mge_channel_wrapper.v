...
module alt_mge_channel_wrapper (
   ...
   // input               rx_serial_data,
    ...
    wire [10:0] csr_xcvr_rcfg_address;
   ...
   wire o_reconfig_eth_readdata_valid;
   wire [31:0] o_reconfig_eth_readdata;
   wire o_reconfig_eth_waitrequest;
   wire o_reconfig_xcvr0_readdata_valid;
    
    // Channel readiness status
    assign xgmii_rx_link_fault_status_good = (xgmii_rx_link_fault_status == 2'b00);
    
//mprash2x
  assign tx_digitalreset = ~i_tx_rst_n;
  assign rx_digitalreset = ~i_rx_rst_n;
   ...
   /* alt_mge_xcvr_reset_ctrl_channel xcvr_reset_ctrl_ch (
    ...
    alt_mge_channel phy (
      .csr_clk                             ( csr_clk                      ),   
       ...
       .i_reconfig_reset                    ( reset         ),  //from synchronizer 
       .o_cdr_lock                          ( o_cdr_lock            ),   // to probe  
       .o_tx_pll_locked                     ( o_tx_pll_locked            ),   //to probe
       .o_sys_pll_locked                    ( o_sys_pll_locked             ),//iopll   
       .o_tx_lanes_stable                   (o_tx_lanes_stable             ),  
       .o_rx_pcs_ready                      (o_rx_pcs_ready             ),   
       .serial_o_tx_serial                  ( o_tx_serial           ),   
       .serial_i_rx_serial                  ( i_rx_serial           ),   
       .serial_o_tx_serial_n                ( o_tx_serial_n         ),   
       .serial_i_rx_serial_n                ( i_rx_serial_n         ),   
       .i_clk_ref                           ( i_clk_ref                    ),   
       .i_clk_sys                           ( i_clk_sys                    ),   
       .i_reconfig_eth_addr                 (  'b0 ),       
       .i_reconfig_eth_byteenable           (  'b0 ),   
       .o_reconfig_eth_readdata_valid       ( o_reconfig_eth_readdata_valid  ),   
       .i_reconfig_eth_read                 (  'b0 ),   
       .i_reconfig_eth_write                ( 'b0   ),   
       .o_reconfig_eth_readdata             ( o_reconfig_eth_readdata  ),   
       .i_reconfig_eth_writedata            ( 'b0  ),   
       .o_reconfig_eth_waitrequest          ( o_reconfig_eth_waitrequest  ),   
       .i_rst_n                             ( i_rst_n                      ),   
       .i_tx_rst_n                          ( i_tx_rst_n                   ),   
       .i_rx_rst_n                          ( i_rx_rst_n                   ),   
       .reconfig_xcvr_slave_0_address       (i_reconfig_xcvr0_address ),   
       .i_reconfig_xcvr0_byteenable         ( 'b1  ),   
       ...
       .i_clk_pll                           ( 'b0                    ),  //only For PTP
        .tx_pma_clk (o_clk_pll),
        .rx_pma_clk (o_clk_pll),
        .i_tx_clk_clk (o_clk_pll),
        .i_rx_clk_clk (o_clk_pll)
